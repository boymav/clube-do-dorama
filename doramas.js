const doramas = [
{
 nome:"Pousando no Amor (Crash Landing on You)",
 nomeEN:"Crash Landing on You",
 nomePT:"Pousando no Amor",
 tags:["triste","romance","intenso"],
 nota:"9.2/10 – MyDramaList",
 elenco:["Hyun Bin", "Son Ye-jin", "Seo Ji-hye", "Kim Jung-hyun", "Yang Kyung-won"],
 descricao:"Uma empresária sul-coreana sofre um acidente e vai parar na Coreia do Norte.\nLá, conhece um oficial sério e protetor.\nUm romance intenso marcado por sacrifícios e emoção.",
 resumo:`Yoon Se-ri é uma empresária rica e bem-sucedida da Coreia do Sul que sofre um acidente de parapente durante uma tempestade.
Os ventos a levam acidentalmente para território da Coreia do Norte.

Ela é encontrada por Ri Jeong-hyeok, um oficial do exército norte-coreano extremamente disciplinado e reservado.
Mesmo sabendo dos riscos, ele decide escondê-la e protegê-la.

Enquanto tentam encontrar uma maneiraa segura de enviá-la de volta para casa,
os dois começam a se aproximar e a criar sentimentos profundos.

A história mostra as diferenças culturais entre as duas Coreias,
a lealdade dos soldados,
e os perigos políticos que os cercam.

Entre momentos de humor, tensão e romance proibido,
o casal enfrenta escolhas difíceis e grandes sacrifícios.

É uma história emocionante sobre amor que ultrapassa fronteiras e ideologias.

No Brasil, esse dorama também é conhecido como "Pousando no Amor".`,
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/main/imagens/doramas/crash-landing.jpg",
 trailer:"https://www.youtube.com/embed/eXMjTXL2Vks"
,
 criticaHumana:`Pousando no Amor costuma funcionar muito bem para quem gosta de romance com bastante emoção e personagens que deixam marca.

A proposta da história já chama atenção logo no começo: Uma empresária sul-coreana sofre um acidente e vai parar na Coreia do Norte.\nLá, conhece um oficial sério e protetor.\nUm romance intenso marcado por sacrifícios e emoção.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Pousando no Amor reúne nomes como Hyun Bin, Son Ye-jin, Seo Ji-hye, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Kim Jung-hyun também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Goblin",
  nomePT:"Goblin: O Solitário e Grande Deus",
  nomeEN:"Goblin",
 tags:["romance","fantasia","triste"],
 nota:"8.8/10 – MyDramaList",
 elenco:["Gong Yoo", "Kim Go-eun", "Lee Dong-wook", "Yoo In-na", "Yook Sung-jae"],
 descricao:"Um ser imortal busca alguém para quebrar sua maldição.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/main/imagens/doramas/Goblin.jpg",
   trailer:"https://www.youtube.com/embed/8AcNEVUzV4o",
resumo:`Kim Shin é um general traído que acaba sendo amaldiçoado com a imortalidade. 
Séculos depois, ele ainda vive entre os humanos carregando culpa e solidão.
A única forma de quebrar a maldição é encontrar sua noiva destinada.
Ji Eun Tak é uma jovem otimista que consegue ver fantasmas.
Ela cruza o caminho do Goblin e passa a mudar sua existência.
Enquanto isso, um ceifador da morte vive ao lado dele, criando situações engraçadas.
Entre humor, dor e romance, o dorama fala sobre destino e sacrifício.
A história mistura fantasia com emoções profundas.
É uma jornada sobre amar mesmo sabendo que pode perder.
Cada episódio constrói uma ligação intensa entre os personagens.
O passado e o presente se conectam de forma surpreendente.
O romance é delicado, mas cheio de sofrimento.
Mostra que até seres imortais desejam uma vida comum.
O final é emocionante e inesquecível.

No Brasil, esse dorama também é conhecido como "Goblin: O Solitário e Grande Deus".`
,
 criticaHumana:`Goblin: O Solitário e Grande Deus costuma funcionar muito bem para quem gosta de romance com bastante emoção e personagens que deixam marca.

A proposta da história já chama atenção logo no começo: Um ser imortal busca alguém para quebrar sua maldição.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Goblin: O Solitário e Grande Deus reúne nomes como Gong Yoo, Kim Go-eun, Lee Dong-wook, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Yoo In-na também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Pretendente Surpresa (Business Proposal)",
 nomeEN:"Business Proposal",
 nomePT:"Pretendente Surpresa",
 tags:["alegre","comedia","romance"],
 nota:"8.6/10 – MyDramaList",
 elenco:["Ahn Hyo-seop", "Kim Se-jeong", "Seol In-ah", "Kim Min-kyu", "Lee Deok-hwa"],
 descricao:"Um encontro às cegas vira confusão quando ela conhece o próprio chefe.\nSituações engraçadas e muito romance.\nLeve, divertido e perfeito para relaxar.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/pretendente%20surpresa.jpg",
 trailer:"https://www.youtube.com/embed/mh4R-WXRhQo"
,
 criticaHumana:`Pretendente Surpresa é uma boa escolha para quem quer um dorama mais leve, divertido e fácil de maratonar.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Pretendente Surpresa reúne nomes como Ahn Hyo-seop, Kim Se-jeong, Seol In-ah, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Kim Min-kyu também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Strong Woman Do Bong Soon",
  nomePT:"Mulher Forte Do Bong-soon",
  nomeEN:"Strong Woman Do Bong Soon",
 tags:["alegre","comedia","romance"],
 nota:"8.7/10 – MyDramaList",
 elenco:["Park Bo-young", "Park Hyung-sik", "Ji Soo", "Ahn Woo-yeon", "Shim Hye-jin"],
 descricao:"Uma jovem nasce com força sobrenatural.\nEla tenta viver normalmente enquanto protege quem ama.\nMistura perfeita de comédia, ação e romance.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/strong-woman-do-bong-soon.jpg",
 trailer:"https://www.youtube.com/embed/znds8S2kKg4"
,
 criticaHumana:`Mulher Forte Do Bong-soon é uma boa escolha para quem quer um dorama mais leve, divertido e fácil de maratonar.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Mulher Forte Do Bong-soon reúne nomes como Park Bo-young, Park Hyung-sik, Ji Soo, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Ahn Woo-yeon também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Tudo Bem Não Ser Normal (Its Okay to Not Be Okay)",
 nomeEN:"Its Okay to Not Be Okay",
 nomePT:"Tudo Bem Não Ser Normal",
 tags:["romance","intenso","triste"],
 nota:"9.0/10 – MyDramaList",
 elenco:["Kim Soo-hyun", "Seo Ye-ji", "Oh Jung-se", "Park Gyu-young", "Kim Joo-hun"],
 descricao:"Um cuidador emocionalmente fechado conhece uma escritora excêntrica.\nAmbos carregam traumas profundos.\nUma história sensível sobre cura e amor.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/tudo%20bem%20nao%20ser%20normal.jpg",
 trailer:"https://www.youtube.com/embed/1H__LNPCc80"
,
 criticaHumana:`Tudo Bem Não Ser Normal costuma funcionar muito bem para quem gosta de romance com bastante emoção e personagens que deixam marca.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Tudo Bem Não Ser Normal reúne nomes como Kim Soo-hyun, Seo Ye-ji, Oh Jung-se, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Park Gyu-young também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Itaewon Class",
 tags:["intenso","romance"],
 nota:"8.9/10 – MyDramaList",
 elenco:["Park Seo-joon", "Kim Da-mi", "Yoo Jae-myung", "Kwon Nara", "Ahn Bo-hyun"],
 descricao:"Um jovem luta para vencer injustiças do passado.\nEle abre um bar em Itaewon com grandes sonhos.\nDrama de superação, amizade e amor.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/itaewon%20class.webp",
 trailer:"https://www.youtube.com/embed/NeaHNQJ1kCo"
,
 criticaHumana:`Itaewon Class é o tipo de dorama que costuma agradar quem procura uma história envolvente e personagens fortes.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Itaewon Class reúne nomes como Park Seo-joon, Kim Da-mi, Yoo Jae-myung, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Kwon Nara também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Vincenzo",
 tags:["alegre","intenso","comedia"],
 nota:"8.8/10 – MyDramaList",
 elenco:["Song Joong-ki", "Jeon Yeo-been", "Ok Taec-yeon", "Kim Yeo-jin", "Kwak Dong-yeon"],
 descricao:"Um advogado da máfia italiana retorna à Coreia.\nEle enfrenta vilões poderosos de forma inusitada.\nAção, humor ácido e justiça.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/vincenzo.jpg",
 trailer:"https://www.youtube.com/embed/S12-4mXCNj4"
,
 criticaHumana:`Vincenzo é uma boa escolha para quem quer um dorama mais leve, divertido e fácil de maratonar.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Vincenzo reúne nomes como Song Joong-ki, Jeon Yeo-been, Ok Taec-yeon, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Kim Yeo-jin também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Quando o elenco funciona bem, a história ganha mais força e os personagens acabam ficando bem mais memoráveis.`
},
{
 nome:"Reply 1988",
 tags:["alegre","romance"],
 nota:"9.1/10 – MyDramaList",
 elenco:["Hyeri", "Ryu Jun-yeol", "Park Bo-gum", "Go Kyung-pyo", "Lee Dong-hwi"],
 descricao:"Famílias e amigos vivem no mesmo bairro nos anos 80.\nHistória cheia de nostalgia e afeto.\nUm dorama sobre crescer e amar.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/reply%201988.jpg",
 trailer:"https://www.youtube.com/embed/hDI4IpZoaG4",
 resumo:`A história acompanha cinco amigos que cresceram juntos em um bairro simples de Seul nos anos 80.
Cada família tem suas dificuldades, sonhos e momentos engraçados.

O dorama mostra o dia a dia cheio de pequenas alegrias,
brigas bobas e muito companheirismo.

Os pais também têm grande destaque,
mostrando o amor silencioso e os sacrifícios feitos pelos filhos.

Entre romances adolescentes e decisões importantes,
os personagens aprendem sobre amizade, família e amadurecimento.

É uma história nostálgica,
emocionante e extremamente acolhedora.

Mostra que felicidade muitas vezes está nas coisas simples da vida.`
,
 criticaHumana:`Reply 1988 é uma boa escolha para quem quer um dorama mais leve, divertido e fácil de maratonar.

A proposta da história já chama atenção logo no começo: Famílias e amigos vivem no mesmo bairro nos anos 80.\nHistória cheia de nostalgia e afeto.\nUm dorama sobre crescer e amar.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Reply 1988 reúne nomes como Hyeri, Ryu Jun-yeol, Park Bo-gum, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Go Kyung-pyo também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Hotel Del Luna",
 tags:["romance","triste","fantasia"],
 nota:"8.9/10 – MyDramaList",
 elenco:["IU", "Yeo Jin-goo", "Shin Jung-geun", "P.O", "Kang Mi-na"],
 descricao:"Um hotel misterioso recebe apenas almas perdidas.\nSua dona carrega uma grande culpa do passado.\nFantasia com romance e emoção.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/hotel%20del%20luna.jpg",
 trailer:"https://www.youtube.com/embed/lJ3_1v8sB48",
 resumo:`O Hotel Del Luna não é um hotel comum.
Ele recebe apenas almas que ainda têm assuntos pendentes antes de partir.

Jang Man Wol é a misteriosa dona do hotel,
presa a uma maldição antiga por causa de erros do passado.

Quando um jovem humano começa a trabalhar no hotel,
ele passa a descobrir segredos escondidos há séculos.

A cada episódio, novas histórias de fantasmas aparecem,
misturando emoção, tristeza e aprendizado.

O romance cresce lentamente,
mas é marcado por destino e sacrifício.

É uma fantasia visualmente linda,
com muita emoção e reflexões sobre arrependimento e perdão.`
,
 criticaHumana:`Hotel Del Luna costuma funcionar muito bem para quem gosta de romance com bastante emoção e personagens que deixam marca.

A proposta da história já chama atenção logo no começo: Um hotel misterioso recebe apenas almas perdidas.\nSua dona carrega uma grande culpa do passado.\nFantasia com romance e emoção.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Hotel Del Luna reúne nomes como IU, Yeo Jin-goo, Shin Jung-geun, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, P.O também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Descendants of the Sun",
  nomePT:"Descendentes do Sol",
  nomeEN:"Descendants of the Sun",
 tags:["romance","intenso"],
 nota:"8.7/10 – MyDramaList",
 elenco:["Song Joong-ki", "Song Hye-kyo", "Jin Goo", "Kim Ji-won", "Onew"],
 descricao:"Um soldado e uma médica se apaixonam em missão humanitária.\nEles enfrentam perigos constantes.\nRomance com ação e drama.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/Descendants%20of%20the%20Sun.jpg",
 trailer:"https://www.youtube.com/embed/wkHjOTFv60g",
 resumo:`Yoo Si Jin é um capitão das forças especiais,
acostumado a viver situações perigosas.

Kang Mo Yeon é uma médica determinada
que acredita salvar vidas é sua maior missão.

Quando os dois se apaixonam,
precisam lidar com escolhas difíceis entre dever e amor.

Enviados para uma zona de guerra,
enfrentam terremotos, doenças e conflitos armados.

O dorama mistura ação intensa com romance emocionante.

Mostra coragem, amizade e sacrifícios,
além de um amor que precisa sobreviver em meio ao caos.

No Brasil, esse dorama também é conhecido como "Descendentes do Sol".`
,
 criticaHumana:`Descendentes do Sol é o tipo de dorama que costuma agradar quem procura uma história envolvente e personagens fortes.

A proposta da história já chama atenção logo no começo: Um soldado e uma médica se apaixonam em missão humanitária.\nEles enfrentam perigos constantes.\nRomance com ação e drama.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Descendentes do Sol reúne nomes como Song Joong-ki, Song Hye-kyo, Jin Goo, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Kim Ji-won também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"My Love from the Star",
  nomePT:"Meu Amor das Estrelas",
  nomeEN:"My Love from the Star",
 tags:["romance","comedia","fantasia"],
 nota:"8.6/10 – MyDramaList",
 elenco:["Kim Soo-hyun", "Jun Ji-hyun", "Park Hae-jin", "Yoo In-na", "Shin Sung-rok"],
 descricao:"Um alienígena vive na Terra há séculos.\nEle se apaixona por uma atriz famosa.\nRomance leve com toques de fantasia.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/You_Who_Came_From_the_Stars_Cover.jpg",
 trailer:"https://www.youtube.com/embed/57Yc6Z2OiC4",
 resumo:`Do Min Joon é um alienígena que chegou à Terra durante a dinastia Joseon.
Ele vive há mais de 400 anos entre os humanos,
sempre mantendo distância para não criar laços.

Frio e racional, ele acredita que o amor é apenas um sentimento passageiro.
Tudo muda quando conhece Cheon Song Yi,
uma atriz famosa, engraçada e completamente diferente dele.

A convivência dos dois começa cheia de conflitos,
mas logo se transforma em algo muito mais profundo.

Enquanto tentam lidar com seus sentimentos,
precisam enfrentar perigos e segredos do passado.

O dorama mistura romance, comédia e fantasia
de uma forma leve e emocionante.

É uma história sobre amar mesmo quando o tempo é limitado.

No Brasil, esse dorama também é conhecido como "Meu Amor das Estrelas".`
,
 criticaHumana:`Meu Amor das Estrelas é uma boa escolha para quem quer um dorama mais leve, divertido e fácil de maratonar.

A proposta da história já chama atenção logo no começo: Um alienígena vive na Terra há séculos.\nEle se apaixona por uma atriz famosa.\nRomance leve com toques de fantasia.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Meu Amor das Estrelas reúne nomes como Kim Soo-hyun, Jun Ji-hyun, Park Hae-jin, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Yoo In-na também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Weightlifting Fairy Kim Bok Joo",
  nomePT:"Fada do Levantamento de Peso Kim Bok-Joo",
  nomeEN:"Weightlifting Fairy Kim Bok Joo",
 tags:["alegre","romance","comedia"],
 nota:"8.8/10 – MyDramaList",
 elenco:["Lee Sung-kyung", "Nam Joo-hyuk", "Kyung Soo-jin", "Lee Jae-yoon", "Cho Hye-jung"],
 descricao:"Uma jovem atleta sonha em ser campeã de levantamento de peso.\nEla vive sua juventude entre amigos e amor.\nDoce, divertido e inspirador.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/Weightlifting%20Fairy%20Kim%20Bok%20Joo.webp",
 trailer:"https://www.youtube.com/embed/6t5dLenA85Y",
 resumo:`Kim Bok Joo é uma estudante universitária e atleta de levantamento de peso.
Ela é forte fisicamente, mas muito sensível por dentro.

Seu maior sonho é conquistar uma medalha de ouro
e deixar sua família orgulhosa.

Ela reencontra Jung Joon Hyung,
um nadador talentoso e brincalhão que estudou com ela na infância.

Entre provocações e momentos engraçados,
os dois começam a desenvolver sentimentos.

O dorama mostra amizade verdadeira,
pressão esportiva e o primeiro amor.

É leve, divertido e extremamente aconchegante.

Uma história simples, mas cheia de coração.

No Brasil, esse dorama também é conhecido como "Fada do Levantamento de Peso Kim Bok-Joo".`
,
 criticaHumana:`Fada do Levantamento de Peso Kim Bok-Joo é uma boa escolha para quem quer um dorama mais leve, divertido e fácil de maratonar.

A proposta da história já chama atenção logo no começo: Uma jovem atleta sonha em ser campeã de levantamento de peso.\nEla vive sua juventude entre amigos e amor.\nDoce, divertido e inspirador.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Fada do Levantamento de Peso Kim Bok-Joo reúne nomes como Lee Sung-kyung, Nam Joo-hyuk, Kyung Soo-jin, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Lee Jae-yoon também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Beleza Verdadeira (True Beauty)",
  nomePT:"Beleza Verdadeira",
  nomeEN:"True Beauty",
 tags:["alegre","romance","comedia"],
 nota:"8.4/10 – MyDramaList",
 elenco:["Moon Ga-young", "Cha Eun-woo", "Hwang In-yeop", "Park Yoo-na", "Jang Hye-jin"],
 descricao:"Uma garota aprende a se maquiar para esconder inseguranças.\nEla vive um triângulo amoroso na escola.\nComédia romântica leve.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/True%20Beauty.jpg",
 trailer:"https://www.youtube.com/embed/-ExizS4OJkg",
 resumo:`Lim Ju Kyung sempre sofreu bullying por causa de sua aparência.
Para recomeçar em uma nova escola,
ela aprende técnicas de maquiagem pela internet.

Com sua nova aparência,
ela se torna extremamente popular.

Mas apenas duas pessoas descobrem seu segredo:
Lee Su Ho e Han Seo Jun.

Entre inseguranças e sentimentos confusos,
ela vive um intenso triângulo amoroso.

O dorama aborda autoestima,
amizade e amadurecimento.

É divertido, romântico e cheio de momentos fofos.

Mostra que beleza verdadeira vai muito além da aparência.

No Brasil, esse dorama também é conhecido como "Beleza Verdadeira".`
,
 criticaHumana:`Beleza Verdadeira é uma boa escolha para quem quer um dorama mais leve, divertido e fácil de maratonar.

A proposta da história já chama atenção logo no começo: Uma garota aprende a se maquiar para esconder inseguranças.\nEla vive um triângulo amoroso na escola.\nComédia romântica leve.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Beleza Verdadeira reúne nomes como Moon Ga-young, Cha Eun-woo, Hwang In-yeop, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Park Yoo-na também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Moon Lovers: Scarlet Heart Ryeo",
  nomePT:"Amantes da Lua: Coração Escarlate Ryeo",
  nomeEN:"Moon Lovers: Scarlet Heart Ryeo",
 tags:["romance","triste","intenso"],
 nota:"8.7/10 – MyDramaList",
 elenco:["Lee Joon-gi", "IU", "Kang Ha-neul", "Hong Jong-hyun", "Nam Joo-hyuk"],
 descricao:"Uma mulher volta no tempo para a era Goryeo.\nEla se envolve com vários príncipes.\nRomance histórico cheio de dor.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/moon%20lovers%201.webp",
 trailer:"https://www.youtube.com/embed/2ljxyaAIphc",
 resumo:`Go Ha Jin vive na Coreia moderna até sofrer um acidente inesperado.
Ao acordar, ela descobre que voltou no tempo para a era Goryeo.

Agora vivendo como Hae Soo,
ela se envolve com vários príncipes da família real.

Entre disputas pelo trono,
ciúmes e traições,
ela se aproxima do misterioso 4º príncipe, Wang So.

O romance dos dois é intenso,
mas marcado por dor e sacrifício.

O dorama mistura história, política e paixão profunda.

É conhecido por seu final emocionante e inesquecível.

No Brasil, esse dorama também é conhecido como "Amantes da Lua: Coração Escarlate Ryeo".`
,
 criticaHumana:`Amantes da Lua: Coração Escarlate Ryeo costuma funcionar muito bem para quem gosta de romance com bastante emoção e personagens que deixam marca.

A proposta da história já chama atenção logo no começo: Uma mulher volta no tempo para a era Goryeo.\nEla se envolve com vários príncipes.\nRomance histórico cheio de dor.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Amantes da Lua: Coração Escarlate Ryeo reúne nomes como Lee Joon-gi, IU, Kang Ha-neul, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Hong Jong-hyun também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"The Heirs",
  nomePT:"Os Herdeiros",
  nomeEN:"The Heirs",
 tags:["romance","triste"],
 nota:"8.1/10 – MyDramaList",
 elenco:["Lee Min-ho", "Park Shin-hye", "Kim Woo-bin", "Kim Ji-won", "Kang Min-hyuk"],
 descricao:"Jovens ricos vivem conflitos amorosos e familiares.\nDiferenças sociais marcam os relacionamentos.\nClássico do romance escolar.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/The%20Heirs.jpg",
 trailer:"https://www.youtube.com/embed/Y0WSF5ikpvU",
 resumo:`Kim Tan é herdeiro de um grande império empresarial.
Ele estuda em uma escola exclusiva para jovens ricos.

Cha Eun Sang é uma garota simples,
que acaba entrando nesse mundo de luxo e competição.

Os dois se apaixonam,
mas enfrentam forte oposição da família dele.

Além do romance,
o dorama mostra rivalidades,
amizades e conflitos de classe social.

É um clássico do romance escolar coreano,
cheio de drama e emoções intensas.

No Brasil, esse dorama também é conhecido como "Os Herdeiros".`
,
 criticaHumana:`Os Herdeiros costuma funcionar muito bem para quem gosta de romance com bastante emoção e personagens que deixam marca.

A proposta da história já chama atenção logo no começo: Jovens ricos vivem conflitos amorosos e familiares.\nDiferenças sociais marcam os relacionamentos.\nClássico do romance escolar.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Os Herdeiros reúne nomes como Lee Min-ho, Park Shin-hye, Kim Woo-bin, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Kim Ji-won também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Start-Up",
 tags:["romance","alegre","intenso"],
 nota:"8.6/10 – MyDramaList",
 elenco:["Bae Suzy", "Nam Joo-hyuk", "Kim Seon-ho", "Kang Han-na", "Kim Hae-sook"],
 descricao:"Jovens tentam realizar seus sonhos no mundo das startups.\nAmor, ambição e desafios se misturam.\nInspirador e emocionante.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/start%20up.jpg",
 trailer:"https://www.youtube.com/embed/BemKyzbLDDc",
 resumo:`Seo Dal Mi sonha em se tornar uma grande empresária.
Ela entra no competitivo mundo das startups coreanas.

Nam Do San é um jovem programador genial,
mas inseguro sobre seu futuro.

Os dois se unem para criar uma empresa inovadora,
enfrentando desafios financeiros e emocionais.

Entre ambição e romance,
eles precisam provar seu valor.

O dorama fala sobre sonhos,
fracassos e perseverança.

É inspirador e cheio de momentos marcantes.`
,
 criticaHumana:`Start-Up é uma boa escolha para quem quer um dorama mais leve, divertido e fácil de maratonar.

A proposta da história já chama atenção logo no começo: Jovens tentam realizar seus sonhos no mundo das startups.\nAmor, ambição e desafios se misturam.\nInspirador e emocionante.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Start-Up reúne nomes como Bae Suzy, Nam Joo-hyuk, Kim Seon-ho, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Kang Han-na também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Hometown Cha-Cha-Cha",
 nomeEN:"Hometown Cha-Cha-Cha",
 nomePT:"Hometown Cha-Cha-Cha",
 tags:["romance","alegre","comedia"],
 nota:"8.9/10 – MyDramaList",
 elenco:["Shin Min-a", "Kim Seon-ho", "Lee Sang-yi", "Gong Min-jung", "In Gyo-jin"],
 descricao:"Uma dentista se muda para uma vila à beira-mar.\nEla conhece um faz-tudo muito carismático.\nRomance leve e aconchegante.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/hometown%20cha.jpg",
 trailer:"https://www.youtube.com/embed/z66fazyp3-M",
 resumo:`Yoon Hye Jin é uma dentista inteligente e determinada.
Após problemas na cidade grande, ela decide recomeçar em uma pequena vila litorânea.

Lá ela conhece Hong Du Sik,
um homem simples conhecido por ajudar todos na cidade.

Apesar das diferenças,
os dois começam a se aproximar.

Entre moradores carismáticos,
histórias emocionantes e momentos divertidos,
o romance cresce de forma leve e natural.

É um dorama aconchegante,
cheio de carinho e crescimento pessoal.`
,
 criticaHumana:`Hometown Cha-Cha-Cha é uma boa escolha para quem quer um dorama mais leve, divertido e fácil de maratonar.

A proposta da história já chama atenção logo no começo: Uma dentista se muda para uma vila à beira-mar.\nEla conhece um faz-tudo muito carismático.\nRomance leve e aconchegante.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Hometown Cha-Cha-Cha reúne nomes como Shin Min-a, Kim Seon-ho, Lee Sang-yi, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Gong Min-jung também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Because This Is My First Life",
 tags:["romance","triste"],
 nota:"8.5/10 – MyDramaList",
 elenco:["Lee Min-ki", "Jung So-min", "Esom", "Park Byung-eun", "Kim Ga-eun"],
 descricao:"Um casamento por contrato muda duas vidas.\nEles aprendem a lidar com sentimentos reais.\nRomance maduro e sensível.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/because%20this%20is.jfif",
 trailer:"https://www.youtube.com/embed/agYjoFB909A",
 resumo:`Nam Se Hee é um homem introvertido que só pensa em pagar sua casa.
Yoon Ji Ho é uma escritora desempregada procurando estabilidade.

Por necessidade,
eles decidem morar juntos sob um contrato de casamento falso.

Com o tempo,
o relacionamento frio começa a se transformar em algo verdadeiro.

O dorama aborda temas adultos como casamento,
carreira e expectativas sociais.

É uma história sensível,
realista e cheia de diálogos profundos.`
,
 criticaHumana:`Because This Is My First Life costuma funcionar muito bem para quem gosta de romance com bastante emoção e personagens que deixam marca.

A proposta da história já chama atenção logo no começo: Um casamento por contrato muda duas vidas.\nEles aprendem a lidar com sentimentos reais.\nRomance maduro e sensível.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Because This Is My First Life reúne nomes como Lee Min-ki, Jung So-min, Esom, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Park Byung-eun também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"W: Two Worlds",
 tags:["romance","fantasia","intenso"],
 nota:"8.7/10 – MyDramaList",
 elenco:["Lee Jong-suk", "Han Hyo-joo", "Jung Yoo-jin", "Lee Tae-hwan", "Park Won-sang"],
 descricao:"Dois mundos se conectam através de um webtoon.\nRealidade e ficção se misturam.\nRomance cheio de ação.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/w%20two%20words.webp",
 trailer:"https://www.youtube.com/embed/4kN0WTI8vWw",
 resumo:`Oh Yeon Joo é uma médica cujo pai é autor de um famoso webtoon.
Um dia, ela é misteriosamente puxada para dentro da história.

Lá ela conhece Kang Chul,
o protagonista do quadrinho.

Os dois tentam entender como os mundos real e fictício se conectam.

Enquanto enfrentam perigos,
o romance cresce entre eles.

Mistura ação,
mistério e fantasia de forma intensa.

É um dorama criativo e cheio de reviravoltas.`
,
 criticaHumana:`W: Two Worlds chama atenção pela mistura de fantasia com emoção, o que deixa a história mais marcante.

A proposta da história já chama atenção logo no começo: Dois mundos se conectam através de um webtoon.\nRealidade e ficção se misturam.\nRomance cheio de ação.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de W: Two Worlds reúne nomes como Lee Jong-suk, Han Hyo-joo, Jung Yoo-jin, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Lee Tae-hwan também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"While You Were Sleeping",
 tags:["romance","fantasia","intenso"],
 nota:"8.8/10 – MyDramaList",
 elenco:["Bae Suzy", "Lee Jong-suk", "Jung Hae-in", "Lee Sang-yeob", "Ko Sung-hee"],
 descricao:"Três pessoas conseguem prever o futuro em sonhos.\nEles tentam evitar tragédias.\nSuspense com romance.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/whire%20you%20we%20sleeping.webp",
 trailer:"https://www.youtube.com/embed/8_rEZV3n3mk",
 resumo:`Nam Hong Joo começa a ter sonhos que mostram tragédias futuras.
Ela percebe que precisa agir para impedir que esses acontecimentos se tornem realidade.

Um promotor justo e um policial dedicado também passam a ter visões semelhantes.
Os três se unem para tentar mudar destinos.

Enquanto enfrentam casos perigosos,
um romance delicado começa a surgir.

O dorama mistura suspense,
emoção e fantasia.

Cada episódio traz tensão e decisões difíceis.

É uma história sobre destino,
coragem e amor inesperado.`
,
 criticaHumana:`While You Were Sleeping chama atenção pela mistura de fantasia com emoção, o que deixa a história mais marcante.

A proposta da história já chama atenção logo no começo: Três pessoas conseguem prever o futuro em sonhos.\nEles tentam evitar tragédias.\nSuspense com romance.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de While You Were Sleeping reúne nomes como Bae Suzy, Lee Jong-suk, Jung Hae-in, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Lee Sang-yeob também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Her Private Life",
 tags:["romance","alegre","comedia"],
 nota:"8.5/10 – MyDramaList",
 elenco:["Park Min-young", "Kim Jae-wook", "Ahn Bo-hyun", "Park Jin-joo", "Kim Mi-kyung"],
 descricao:"Uma curadora de arte leva uma vida dupla.\nEla é fã secreta de um idol.\nRomance divertido e leve.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/her%20private%20life.jpg",
 trailer:"https://www.youtube.com/embed/V_QJ0jLKWFo",
 resumo:`Sung Deok Mi é uma curadora de arte profissional e respeitada.
Mas fora do trabalho, ela vive secretamente como fã obsessiva de um idol.

Quando um novo diretor começa a trabalhar no museu,
ele descobre seu segredo.

Entre mal-entendidos e situações engraçadas,
os dois começam a se aproximar.

O dorama mostra o lado divertido do mundo dos fãs.

É leve,
romântico e cheio de química entre o casal principal.

Perfeito para quem quer algo doce e divertido.`
,
 criticaHumana:`Her Private Life é uma boa escolha para quem quer um dorama mais leve, divertido e fácil de maratonar.

A proposta da história já chama atenção logo no começo: Uma curadora de arte leva uma vida dupla.\nEla é fã secreta de um idol.\nRomance divertido e leve.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Her Private Life reúne nomes como Park Min-young, Kim Jae-wook, Ahn Bo-hyun, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Park Jin-joo também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Touch Your Heart",
 tags:["romance","alegre","comedia"],
 nota:"8.4/10 – MyDramaList",
 elenco:["Yoo In-na", "Lee Dong-wook", "Lee Sang-woo", "Son Sung-yoon", "Oh Jung-se"],
 descricao:"Uma atriz trabalha como secretária temporária.\nEla se envolve com um advogado sério.\nDoce e romântico.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/touch%20your%20heartt.jpg",
 trailer:"https://www.youtube.com/embed/7Sdq9d3aN3g",
 resumo:`Oh Yoon Seo é uma atriz que precisa recuperar sua reputação.
Para isso, ela começa a trabalhar como secretária em um escritório de advocacia.

Lá ela conhece Kwon Jung Rok,
um advogado sério e extremamente profissional.

No início, eles vivem muitos conflitos.
Mas aos poucos, a convivência aproxima os dois.

O romance cresce de forma suave e encantadora.

É um dorama leve,
cheio de momentos fofos e carinho.

Ideal para quem ama romance puro.`
,
 criticaHumana:`Touch Your Heart é uma boa escolha para quem quer um dorama mais leve, divertido e fácil de maratonar.

A proposta da história já chama atenção logo no começo: Uma atriz trabalha como secretária temporária.\nEla se envolve com um advogado sério.\nDoce e romântico.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Touch Your Heart reúne nomes como Yoo In-na, Lee Dong-wook, Lee Sang-woo, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Son Sung-yoon também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"My ID is Gangnam Beauty",
 tags:["romance","alegre"],
 nota:"8.2/10 – MyDramaList",
 elenco:["Im Soo-hyang", "Cha Eun-woo", "Jo Woo-ri", "Kwak Dong-yeon", "Min Do-hee"],
 descricao:"Uma jovem muda sua aparência para recomeçar.\nEla enfrenta preconceitos na faculdade.\nMensagem forte sobre autoestima.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/my%20id%20is%20gangnam.webp",
 trailer:"https://www.youtube.com/embed/pBwTLwHzV8I",
 resumo:`Kang Mi Rae sofre bullying durante toda a vida por causa de sua aparência.
Ao entrar na universidade, ela decide fazer cirurgia plástica para recomeçar.

Apesar da mudança, ela percebe que ainda enfrenta julgamentos.
Alguns colegas a chamam de “Gangnam Beauty”, termo usado de forma preconceituosa.

Cha Eun Woo interpreta um estudante popular que enxerga além da aparência.
Ele começa a se aproximar dela de maneira sincera.

O dorama aborda autoestima,
padrões de beleza e amadurecimento.

É uma história romântica,
leve e reflexiva sobre amor próprio.`
,
 criticaHumana:`My ID is Gangnam Beauty é uma boa escolha para quem quer um dorama mais leve, divertido e fácil de maratonar.

A proposta da história já chama atenção logo no começo: Uma jovem muda sua aparência para recomeçar.\nEla enfrenta preconceitos na faculdade.\nMensagem forte sobre autoestima.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de My ID is Gangnam Beauty reúne nomes como Im Soo-hyang, Cha Eun-woo, Jo Woo-ri, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Kwak Dong-yeon também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"18 Again",
 tags:["romance","triste","alegre"],
 nota:"8.7/10 – MyDramaList",
 elenco:["Kim Ha-neul", "Yoon Sang-hyun", "Lee Do-hyun", "Roh Jeong-eui", "Lee Mi-do"],
 descricao:"Um homem volta ao corpo de seus 18 anos.\nEle tenta consertar erros do passado.\nComédia com emoção.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/18againn.jpg",
 trailer:"https://www.youtube.com/embed/neLAy_Gpc-Y",
 resumo:`Hong Dae Young sente que sua vida está desmoronando.
Seu casamento está em crise e ele perdeu o emprego.

De forma misteriosa, ele volta ao corpo de seus 18 anos.
Com essa segunda chance, decide se aproximar novamente dos filhos.

Enquanto vive como adolescente,
ele aprende a enxergar sua família de outra forma.

O dorama mistura comédia,
emoção e reflexão.

Mostra arrependimentos,
amor familiar e crescimento pessoal.

Uma história sensível sobre segundas chances.`
,
 criticaHumana:`18 Again costuma funcionar muito bem para quem gosta de romance com bastante emoção e personagens que deixam marca.

A proposta da história já chama atenção logo no começo: Um homem volta ao corpo de seus 18 anos.\nEle tenta consertar erros do passado.\nComédia com emoção.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de 18 Again reúne nomes como Kim Ha-neul, Yoon Sang-hyun, Lee Do-hyun, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Roh Jeong-eui também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Advogada Extraordinária (Extraordinary Attorney Woo)",
 nomeEN:"Extraordinary Attorney Woo",
 nomePT:"Advogada Extraordinária",
 tags:["alegre","romance"],
 nota:"9.0/10 – MyDramaList",
 elenco:["Park Eun-bin", "Kang Tae-oh", "Kang Ki-young", "Jeon Bae-su", "Joo Hyun-young"],
 descricao:"Uma advogada brilhante dentro do espectro autista.\nEla enfrenta desafios no trabalho.\nDoce, humano e inspirador.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/extraordinay%20attorney.jpg",
 trailer:"https://www.youtube.com/embed/MxeXECe2t-c",
 resumo:`Woo Young Woo é uma advogada extremamente inteligente.
Ela está dentro do espectro autista e possui memória fotográfica.

Ao começar a trabalhar em um grande escritório,
precisa enfrentar preconceitos e desafios sociais.

Cada caso traz aprendizado e crescimento pessoal.
Seus colegas começam a entender melhor suas diferenças.

O romance surge de maneira delicada e respeitosa.

O dorama é emocionante,
humano e inspirador.

Mostra inclusão,
superação e empatia.

No Brasil, esse dorama também é conhecido como "Advogada Extraordinária".`
,
 criticaHumana:`Advogada Extraordinária é uma boa escolha para quem quer um dorama mais leve, divertido e fácil de maratonar.

A proposta da história já chama atenção logo no começo: Uma advogada brilhante dentro do espectro autista.\nEla enfrenta desafios no trabalho.\nDoce, humano e inspirador.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Advogada Extraordinária reúne nomes como Park Eun-bin, Kang Tae-oh, Kang Ki-young, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Jeon Bae-su também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"The King: Eternal Monarch",
 tags:["romance","fantasia","intenso"],
 nota:"8.3/10 – MyDramaList",
 elenco:["Lee Min-ho", "Kim Go-eun", "Woo Do-hwan", "Kim Kyung-nam", "Jung Eun-chae"],
 descricao:"Universos paralelos se conectam.\nUm rei busca salvar seu mundo.\nRomance épico com fantasia.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/the%20king%20eternalllll.jpg",
 trailer:"https://www.youtube.com/embed/N1L1iaFZQ9I",
 resumo:`Lee Gon é o imperador da Coreia em um universo paralelo.
Ele descobre a existência de uma porta que liga dois mundos.

Ao atravessar para a Coreia moderna,
conhece uma detetive determinada.

Enquanto tentam fechar essa passagem perigosa,
um romance nasce entre realidades diferentes.

O vilão ameaça destruir os dois mundos.

O dorama mistura ação,
fantasia e romance intenso.

É uma história épica sobre destino e amor impossível.`
,
 criticaHumana:`The King: Eternal Monarch chama atenção pela mistura de fantasia com emoção, o que deixa a história mais marcante.

A proposta da história já chama atenção logo no começo: Universos paralelos se conectam.\nUm rei busca salvar seu mundo.\nRomance épico com fantasia.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de The King: Eternal Monarch reúne nomes como Lee Min-ho, Kim Go-eun, Woo Do-hwan, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Kim Kyung-nam também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Mr. Sunshine",
 tags:["triste","intenso","romance"],
 nota:"9.1/10 – MyDramaList",
 elenco:["Lee Byung-hun", "Kim Tae-ri", "Yoo Yeon-seok", "Byun Yo-han", "Kim Min-jung"],
 descricao:"Um coreano retorna como soldado americano.\nEle vive um amor impossível.\nDrama histórico poderoso.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/mr%20sunshine.webp",
 trailer:"https://www.youtube.com/embed/rPJSo4fhtRU",
 resumo:`Eugene Choi nasceu escravo na Coreia e fugiu para os Estados Unidos.
Anos depois, retorna como oficial do exército americano.

Ele reencontra sua terra natal em meio a conflitos políticos.
Durante esse período turbulento, conhece Go Ae Shin,
uma nobre que luta secretamente pela independência.

Entre batalhas, traições e escolhas difíceis,
um romance impossível floresce.

O dorama é intenso e cinematográfico.
Mostra patriotismo, dor e sacrifício.

Uma história profunda e emocionante.`
,
 criticaHumana:`Mr. Sunshine costuma funcionar muito bem para quem gosta de romance com bastante emoção e personagens que deixam marca.

A proposta da história já chama atenção logo no começo: Um coreano retorna como soldado americano.\nEle vive um amor impossível.\nDrama histórico poderoso.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Mr. Sunshine reúne nomes como Lee Byung-hun, Kim Tae-ri, Yoo Yeon-seok, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Byun Yo-han também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Healer",
 tags:["romance","intenso"],
 nota:"8.9/10 – MyDramaList",
 elenco:["Ji Chang-wook", "Park Min-young", "Yoo Ji-tae", "Kim Mi-kyung", "Park Sang-won"],
 descricao:"Um mensageiro misterioso trabalha nas sombras.\nEle se envolve com uma jornalista.\nAção, romance e suspense.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/healer.webp",
 trailer:"https://www.youtube.com/embed/EcOhZLULyFA",
 resumo:`Seo Jung Hoo é conhecido como “Healer”.
Ele realiza missões secretas e vive isolado.

Ao aceitar um novo trabalho,
acaba se envolvendo com uma jovem jornalista.

Enquanto investiga um caso antigo,
segredos do passado começam a surgir.

A dupla enfrenta conspirações perigosas.
O romance cresce em meio à tensão.

O dorama mistura ação,
mistério e química intensa entre o casal.

É viciante do começo ao fim.`
,
 criticaHumana:`Healer é o tipo de dorama que costuma agradar quem procura uma história envolvente e personagens fortes.

A proposta da história já chama atenção logo no começo: Um mensageiro misterioso trabalha nas sombras.\nEle se envolve com uma jornalista.\nAção, romance e suspense.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Healer reúne nomes como Ji Chang-wook, Park Min-young, Yoo Ji-tae, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Kim Mi-kyung também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Coffee Prince",
 tags:["romance","alegre","comedia"],
 nota:"8.6/10 – MyDramaList",
 elenco:["Gong Yoo", "Yoon Eun-hye", "Lee Sun-kyun", "Chae Jung-an", "Kim Jae-wook"],
 descricao:"Uma garota se passa por garoto para trabalhar.\nUm romance inesperado surge.\nClássico divertido e romântico.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/coffeee.jpg",
 trailer:"https://www.youtube.com/embed/fpYlv_TNVzM",
 resumo:`Go Eun Chan se disfarça de homem para conseguir emprego.
Ela começa a trabalhar em um café onde todos os funcionários são homens.

O dono do café começa a desenvolver sentimentos por ela,
acreditando que ela seja um rapaz.

Isso cria situações engraçadas e confusas.

Com o tempo, a verdade vem à tona.
O romance é doce e natural.

É um clássico dos doramas,
leve, divertido e cheio de química.`
,
 criticaHumana:`Coffee Prince é uma boa escolha para quem quer um dorama mais leve, divertido e fácil de maratonar.

A proposta da história já chama atenção logo no começo: Uma garota se passa por garoto para trabalhar.\nUm romance inesperado surge.\nClássico divertido e romântico.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Coffee Prince reúne nomes como Gong Yoo, Yoon Eun-hye, Lee Sun-kyun, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Chae Jung-an também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Secret Garden",
 tags:["romance","fantasia","alegre"],
 nota:"8.2/10 – MyDramaList",
 elenco:["Hyun Bin", "Ha Ji-won", "Yoon Sang-hyun", "Kim Sa-rang", "Lee Philip"],
 descricao:"Um casal troca de corpos misteriosamente.\nEles aprendem a se entender.\nRomance clássico com fantasia.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/secret%20garden.jpg",
 trailer:"https://www.youtube.com/embed/9xMLDDm7oGU",
 resumo:`Kim Joo Won é um empresário arrogante.
Gil Ra Im é uma dublê humilde e corajosa.

Após um encontro misterioso,
os dois começam a trocar de corpos.

A situação inusitada os obriga a enxergar o mundo pela perspectiva do outro.

Entre comédia, brigas e romance,
os sentimentos crescem.

O dorama mistura fantasia com romance clássico.

É envolvente e cheio de momentos marcantes.`
,
 criticaHumana:`Secret Garden é uma boa escolha para quem quer um dorama mais leve, divertido e fácil de maratonar.

A proposta da história já chama atenção logo no começo: Um casal troca de corpos misteriosamente.\nEles aprendem a se entender.\nRomance clássico com fantasia.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Secret Garden reúne nomes como Hyun Bin, Ha Ji-won, Yoon Sang-hyun, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Kim Sa-rang também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"The Legend of the Blue Sea",
 tags:["romance","fantasia","comedia"],
 nota:"8.6/10 – MyDramaList",
 elenco:["Jun Ji-hyun", "Lee Min-ho", "Lee Hee-joon", "Shin Won-ho", "Shin Hye-sun"],
 descricao:"Uma sereia chega ao mundo humano.\nEla se apaixona por um golpista.\nRomance divertido e mágico.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/the%20legend%20of.jfif",
 trailer:"https://www.youtube.com/embed/G_jt3EwBMwU",
 resumo:`Shim Cheong é uma sereia que acaba no mundo moderno.
Ela conhece Heo Joon Jae, um golpista charmoso.

Mesmo vindo de mundos diferentes,
os dois criam uma ligação especial.

Segredos do passado conectam suas vidas.
O romance é leve e divertido.

Mistura fantasia, humor e emoção.
Uma história encantadora e apaixonante.`
,
 criticaHumana:`The Legend of the Blue Sea é uma boa escolha para quem quer um dorama mais leve, divertido e fácil de maratonar.

A proposta da história já chama atenção logo no começo: Uma sereia chega ao mundo humano.\nEla se apaixona por um golpista.\nRomance divertido e mágico.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de The Legend of the Blue Sea reúne nomes como Jun Ji-hyun, Lee Min-ho, Lee Hee-joon, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Shin Won-ho também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Doom at Your Service",
 tags:["romance","fantasia","triste"],
 nota:"8.4/10 – MyDramaList",
 elenco:["Park Bo-young", "Seo In-guk", "Lee Soo-hyuk", "Kang Tae-oh", "Shin Do-hyun"],
 descricao:"Uma mulher faz um contrato com a própria destruição.\nEles vivem um amor impossível.\nRomance melancólico.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/doom%20at%20your.jfif",
 trailer:"https://www.youtube.com/embed/C4wFwQYEses",
 resumo:`Tak Dong Kyung descobre que tem pouco tempo de vida.
Em desespero, deseja que o mundo acabe.

Seu pedido chama Myul Mang,
a própria personificação da destruição.

Eles fazem um contrato perigoso.
Mas sentimentos começam a surgir.

É um romance intenso e melancólico.
Fala sobre vida, dor e esperança.`
,
 criticaHumana:`Doom at Your Service costuma funcionar muito bem para quem gosta de romance com bastante emoção e personagens que deixam marca.

A proposta da história já chama atenção logo no começo: Uma mulher faz um contrato com a própria destruição.\nEles vivem um amor impossível.\nRomance melancólico.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Doom at Your Service reúne nomes como Park Bo-young, Seo In-guk, Lee Soo-hyuk, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Kang Tae-oh também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Our Beloved Summer",
 tags:["romance","triste","alegre"],
 nota:"8.8/10 – MyDramaList",
 elenco:["Choi Woo-shik", "Kim Da-mi", "Kim Sung-cheol", "Roh Jeong-eui", "Park Jin-joo"],
 descricao:"Ex-namorados se reencontram anos depois.\nSentimentos mal resolvidos voltam.\nRomance realista e sensível.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/our%20beloved%20summer.png",
 trailer:"https://www.youtube.com/embed/wpW6aVWgvMc",
 resumo:`Choi Ung e Kook Yeon Soo foram namorados na juventude.
Após o término, prometeram nunca mais se ver.

Anos depois, um documentário antigo viraliza.
Eles precisam trabalhar juntos novamente.

Sentimentos mal resolvidos voltam à tona.
O romance é maduro e realista.

Mostra crescimento pessoal e reconciliação.
Delicado e emocionante.`
,
 criticaHumana:`Our Beloved Summer costuma funcionar muito bem para quem gosta de romance com bastante emoção e personagens que deixam marca.

A proposta da história já chama atenção logo no começo: Ex-namorados se reencontram anos depois.\nSentimentos mal resolvidos voltam.\nRomance realista e sensível.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Our Beloved Summer reúne nomes como Choi Woo-shik, Kim Da-mi, Kim Sung-cheol, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Roh Jeong-eui também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Flower of Evil",
  nomePT:"Flor do Mal",
  nomeEN:"Flower of Evil",
 tags:["intenso","romance","triste"],
 nota:"9.1/10 – MyDramaList",
 elenco:["Lee Joon-gi", "Moon Chae-won", "Seo Hyun-woo", "Jang Hee-jin", "Nam Gi-ae"],
 descricao:"Um homem esconde um passado sombrio.\nSua esposa é detetive.\nSuspense psicológico poderoso.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/flower%20of%20evil.webp",
 trailer:"https://www.youtube.com/embed/C1Cp24Ngdfc",
 resumo:`Baek Hee Sung parece ter uma vida perfeita.
Mas ele esconde um passado perigoso.

Sua esposa é detetive e investiga um caso ligado a ele.
A tensão cresce a cada episódio.

O dorama mistura romance com suspense psicológico.
Questiona confiança e identidade.

É intenso e cheio de reviravoltas.

No Brasil, esse dorama também é conhecido como "Flor do Mal".`
,
 criticaHumana:`Flor do Mal costuma funcionar muito bem para quem gosta de romance com bastante emoção e personagens que deixam marca.

A proposta da história já chama atenção logo no começo: Um homem esconde um passado sombrio.\nSua esposa é detetive.\nSuspense psicológico poderoso.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Flor do Mal reúne nomes como Lee Joon-gi, Moon Chae-won, Seo Hyun-woo, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Jang Hee-jin também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Sky Castle",
 tags:["intenso","triste"],
 nota:"8.9/10 – MyDramaList",
 elenco:["Yum Jung-ah", "Lee Tae-ran", "Yoon Se-ah", "Oh Na-ra", "Kim Seo-hyung"],
 descricao:"Famílias ricas disputam o futuro dos filhos.\nPressão extrema e ambição.\nCrítica social forte.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/sky%20castle.webp",
 trailer:"https://www.youtube.com/embed/P6coIgyV9e0",
 resumo:`Famílias da elite vivem em um condomínio luxuoso.
Elas fazem de tudo para garantir o sucesso acadêmico dos filhos.

A pressão é extrema.
Segredos e manipulações surgem.

O dorama critica o sistema educacional.
Mostra ambição, obsessão e consequências.

É forte, realista e impactante.`
,
 criticaHumana:`Sky Castle é o tipo de dorama que costuma agradar quem procura uma história envolvente e personagens fortes.

A proposta da história já chama atenção logo no começo: Famílias ricas disputam o futuro dos filhos.\nPressão extrema e ambição.\nCrítica social forte.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Sky Castle reúne nomes como Yum Jung-ah, Lee Tae-ran, Yoon Se-ah, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Oh Na-ra também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Quando o elenco funciona bem, a história ganha mais força e os personagens acabam ficando bem mais memoráveis.`
},
{
 nome:"Prison Playbook",
 tags:["alegre","triste"],
 nota:"8.8/10 – MyDramaList",
 elenco:["Park Hae-soo", "Jung Kyung-ho", "Krystal Jung", "Choi Moo-sung", "Jung Woong-in"],
 descricao:"Um jogador de beisebol vai parar na prisão.\nHistórias humanas se cruzam.\nDrama com humor e emoção.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/prison%20playbook.jfif",
 trailer:"https://www.youtube.com/embed/MAyHcIzvjnY",
 resumo:`Kim Je Hyuk é um famoso jogador de beisebol.
De repente, ele vai parar na prisão.

Lá conhece detentos com histórias muito diferentes.
Entre momentos engraçados e emocionantes,
ele aprende sobre amizade e humanidade.

O dorama mostra o lado humano da prisão.
Equilibra drama com humor inteligente.

É envolvente e surpreendente.`
,
 criticaHumana:`Prison Playbook é uma boa escolha para quem quer um dorama mais leve, divertido e fácil de maratonar.

A proposta da história já chama atenção logo no começo: Um jogador de beisebol vai parar na prisão.\nHistórias humanas se cruzam.\nDrama com humor e emoção.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Prison Playbook reúne nomes como Park Hae-soo, Jung Kyung-ho, Krystal Jung, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Choi Moo-sung também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Quando o elenco funciona bem, a história ganha mais força e os personagens acabam ficando bem mais memoráveis.`
},
{
 nome:"Because of Meeting You",
 tags:["romance","alegre"],
 nota:"8.3/10 – MyDramaList",
 elenco:["Sun Yi", "Lu Yi", "Zhang Duo", "Wu You", "Jiang Yan"],
 descricao:"Destinos se cruzam de forma inesperada.\nLaços se fortalecem.\nRomance simples e agradável.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/because%20of%20meeting%20you.jpg",
 trailer:"https://www.youtube.com/embed/lWQIoNiziI8",
 resumo:`Duas famílias têm seus destinos ligados por segredos do passado.
Uma jovem luta para descobrir sua verdadeira identidade.

Entre desafios e revelações,
um romance nasce de forma inesperada.

O dorama fala sobre laços familiares.
Mostra superação e amor verdadeiro.

É leve, dramático e emocionante.`
,
 criticaHumana:`Because of Meeting You é uma boa escolha para quem quer um dorama mais leve, divertido e fácil de maratonar.

A proposta da história já chama atenção logo no começo: Destinos se cruzam de forma inesperada.\nLaços se fortalecem.\nRomance simples e agradável.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Because of Meeting You reúne nomes como Sun Yi, Lu Yi, Zhang Duo, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Wu You também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Arthdal Chronicles",
 tags:["fantasia","intenso"],
 nota:"8.4/10 – MyDramaList",
 elenco:["Song Joong-ki", "Jang Dong-gun", "Kim Ji-won", "Kim Ok-vin", "Park Hae-joon"],
 descricao:"Um mundo antigo marcado por guerras e poder.\nDestinos se entrelaçam.\nÉpico histórico.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/arthdal%20chronicles.jpg",
 trailer:"https://www.youtube.com/embed/nWf3aEvyR5k",
 resumo:`Em um mundo antigo chamado Arthdal,
tribos lutam por poder e sobrevivência.

Eun Seom cresce com um destino misterioso.
Conspirações políticas surgem.

O dorama mistura fantasia com história.
Tem batalhas, traições e romances.

É grandioso e cheio de tensão.`
,
 criticaHumana:`Arthdal Chronicles chama atenção pela mistura de fantasia com emoção, o que deixa a história mais marcante.

A proposta da história já chama atenção logo no começo: Um mundo antigo marcado por guerras e poder.\nDestinos se entrelaçam.\nÉpico histórico.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Arthdal Chronicles reúne nomes como Song Joong-ki, Jang Dong-gun, Kim Ji-won, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Kim Ok-vin também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Quando o elenco funciona bem, a história ganha mais força e os personagens acabam ficando bem mais memoráveis.`
},
{
 nome:"My Mister",
 tags:["triste","intenso"],
 nota:"9.2/10 – MyDramaList",
 elenco:["Lee Sun-kyun", "IU", "Lee Ji-ah", "Jang Ki-yong", "Park Ho-san"],
 descricao:"Duas almas feridas se apoiam.\nVida adulta com dores reais.\nProfundo e tocante.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/my%20mister.jfif",
 trailer:"https://www.youtube.com/embed/f-Eaa3f3gDo",
 resumo:`Park Dong Hoon é um homem comum,
sobrecarregado por problemas da vida adulta.

Lee Ji An é uma jovem cheia de dores.
Os dois criam uma conexão inesperada.

Sem romance exagerado,
o dorama foca em empatia e apoio.

Mostra como pequenas gentilezas salvam vidas.
É profundo e extremamente emocionante.`
,
 criticaHumana:`My Mister é o tipo de dorama que costuma agradar quem procura uma história envolvente e personagens fortes.

A proposta da história já chama atenção logo no começo: Duas almas feridas se apoiam.\nVida adulta com dores reais.\nProfundo e tocante.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de My Mister reúne nomes como Lee Sun-kyun, IU, Lee Ji-ah, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Jang Ki-yong também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Quando o elenco funciona bem, a história ganha mais força e os personagens acabam ficando bem mais memoráveis.`
},
{
 nome:"Fight For My Way",
 tags:["alegre","romance"],
 nota:"8.7/10 – MyDramaList",
 elenco:["Park Seo-joon", "Kim Ji-won", "Ahn Jae-hong", "Song Ha-yoon", "Kim Sung-oh"],
 descricao:"Amigos lutam por seus sonhos.\nRomance nasce aos poucos.\nMotivador e divertido.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/flight%20for%20my%20way.webp",
 trailer:"https://www.youtube.com/embed/Xsjj62GcyDo",
 resumo:`Ko Dong Man e Choi Ae Ra são amigos de infância.
Eles sonham em alcançar grandes objetivos.

Entre fracassos e tentativas,
um romance começa a surgir.

O dorama fala sobre persistência.
Mostra amizade verdadeira e amor sincero.

É leve, divertido e inspirador.`
,
 criticaHumana:`Fight For My Way é uma boa escolha para quem quer um dorama mais leve, divertido e fácil de maratonar.

A proposta da história já chama atenção logo no começo: Amigos lutam por seus sonhos.\nRomance nasce aos poucos.\nMotivador e divertido.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Fight For My Way reúne nomes como Park Seo-joon, Kim Ji-won, Ahn Jae-hong, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Song Ha-yoon também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Hospital Playlist",
 tags:["alegre","romance"],
 nota:"9.1/10 – MyDramaList",
 elenco:["Jo Jung-suk", "Yoo Yeon-seok", "Jung Kyung-ho", "Kim Dae-myung", "Jeon Mi-do"],
 descricao:"Cinco médicos são amigos há anos.\nEles dividem a rotina do hospital e a vida pessoal.\nHumano, leve e emocionante.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/hospital%20playlist.webp",
 trailer:"https://www.youtube.com/embed/xqIM2cenBAs",
 resumo:`Cinco médicos são amigos desde a faculdade.
Eles trabalham no mesmo hospital.

Entre cirurgias e plantões,
dividem alegrias e dificuldades da vida adulta.

O dorama mostra amizade verdadeira.
Tem romance leve e momentos emocionantes.

É confortável, humano e cheio de carinho.`
,
 criticaHumana:`Hospital Playlist é uma boa escolha para quem quer um dorama mais leve, divertido e fácil de maratonar.

A proposta da história já chama atenção logo no começo: Cinco médicos são amigos há anos.\nEles dividem a rotina do hospital e a vida pessoal.\nHumano, leve e emocionante.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Hospital Playlist reúne nomes como Jo Jung-suk, Yoo Yeon-seok, Jung Kyung-ho, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Kim Dae-myung também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Nevertheless",
 tags:["romance","triste"],
 nota:"7.6/10 – MyDramaList",
 elenco:["Han So-hee", "Song Kang", "Chae Jong-hyeop", "Yang Hye-ji", "Lee Yul-eum"],
 descricao:"Um romance intenso e complicado.\nSentimentos confusos dominam o casal.\nRealista e melancólico.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/nevert.jfif",
 trailer:"https://www.youtube.com/embed/0x8cJUD6MMo",
 resumo:`Yoo Na Bi não acredita mais no amor.
Park Jae Eon gosta de flertar, mas evita compromisso.

Mesmo sabendo dos riscos,
os dois se aproximam.

O dorama mostra relações modernas.
Sentimentos confusos e inseguranças reais.

É intenso e melancólico.`
,
 criticaHumana:`Nevertheless costuma funcionar muito bem para quem gosta de romance com bastante emoção e personagens que deixam marca.

A proposta da história já chama atenção logo no começo: Um romance intenso e complicado.\nSentimentos confusos dominam o casal.\nRealista e melancólico.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Nevertheless reúne nomes como Han So-hee, Song Kang, Chae Jong-hyeop, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Yang Hye-ji também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Love Alarm",
 tags:["romance","alegre"],
 nota:"7.5/10 – MyDramaList",
 elenco:["Kim So-hyun", "Jung Ga-ram", "Song Kang", "Go Min-si", "Shin Seung-ho"],
 descricao:"Um aplicativo revela quem gosta de você.\nTriângulos amorosos surgem.\nRomance jovem.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/love%20alarm.webp",
 trailer:"https://www.youtube.com/embed/8sXTfzaLmiQ",
 resumo:`Um aplicativo chamado Love Alarm
avisa quando alguém próximo gosta de você.

Kim Jo Jo se envolve em um triângulo amoroso.
O aplicativo muda a forma de se relacionar.

O dorama fala sobre insegurança.
Mostra como tecnologia influencia o amor.

É juvenil e romântico.`
,
 criticaHumana:`Love Alarm é uma boa escolha para quem quer um dorama mais leve, divertido e fácil de maratonar.

A proposta da história já chama atenção logo no começo: Um aplicativo revela quem gosta de você.\nTriângulos amorosos surgem.\nRomance jovem.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Love Alarm reúne nomes como Kim So-hyun, Jung Ga-ram, Song Kang, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Go Min-si também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Snowdrop",
 tags:["romance","triste","intenso"],
 nota:"8.4/10 – MyDramaList",
 elenco:["Jung Hae-in", "Jisoo", "Yoo In-na", "Jang Seung-jo", "Kim Hye-yoon"],
 descricao:"Amor proibido em tempos políticos difíceis.\nSegredos e sacrifícios.\nDrama intenso.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/snowdrop.webp",
 trailer:"https://www.youtube.com/embed/ff0dEyYiYi0",
 resumo:`Eun Young Ro encontra um homem ferido
e decide escondê-lo em seu dormitório.

Ele guarda um segredo perigoso.
O país vive um momento político tenso.

O romance cresce em meio ao caos.
Sacrifícios e escolhas difíceis surgem.

É dramático e emocionante.`
,
 criticaHumana:`Snowdrop costuma funcionar muito bem para quem gosta de romance com bastante emoção e personagens que deixam marca.

A proposta da história já chama atenção logo no começo: Amor proibido em tempos políticos difíceis.\nSegredos e sacrifícios.\nDrama intenso.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Snowdrop reúne nomes como Jung Hae-in, Jisoo, Yoo In-na, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Jang Seung-jo também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Mouse",
 tags:["intenso","triste"],
 nota:"8.9/10 – MyDramaList",
 elenco:["Lee Seung-gi", "Lee Hee-joon", "Park Ju-hyun", "Kyung Soo-jin", "Pyo Ji-hoon"],
 descricao:"Um serial killer assombra a Coreia.\nQuestões morais profundas surgem.\nSuspense psicológico pesado.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/mouse.jpg",
 trailer:"https://www.youtube.com/embed/Q6Nki1_8RBU",
 resumo:`Um assassino em série começa a espalhar terror.
A polícia tenta capturá-lo a qualquer custo.

Um jovem policial se envolve no caso.
Segredos começam a aparecer.

O dorama questiona se alguém nasce mau.
É cheio de reviravoltas e tensão.

Pesado, psicológico e surpreendente.`
,
 criticaHumana:`Mouse é o tipo de dorama que costuma agradar quem procura uma história envolvente e personagens fortes.

A proposta da história já chama atenção logo no começo: Um serial killer assombra a Coreia.\nQuestões morais profundas surgem.\nSuspense psicológico pesado.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Mouse reúne nomes como Lee Seung-gi, Lee Hee-joon, Park Ju-hyun, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Kyung Soo-jin também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Quando o elenco funciona bem, a história ganha mais força e os personagens acabam ficando bem mais memoráveis.`
},
{
 nome:"Signal",
 tags:["intenso","triste"],
 nota:"9.0/10 – MyDramaList",
 elenco:["Lee Je-hoon", "Kim Hye-soo", "Cho Jin-woong", "Jang Hyun-sung", "Jung Hae-kyun"],
 descricao:"Um walkie-talkie liga passado e presente.\nCasos antigos são reabertos.\nSuspense brilhante.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/signal.jpg",
 trailer:"https://www.youtube.com/embed/RsdJLm7Swkw",
 resumo:`Um detetive encontra um rádio misterioso.
Ele consegue falar com um policial do passado.

Juntos, tentam resolver crimes antigos.
Cada decisão altera o futuro.

É inteligente e emocionante.
Baseado em casos reais.

Um dos melhores suspenses coreanos.`
,
 criticaHumana:`Signal é o tipo de dorama que costuma agradar quem procura uma história envolvente e personagens fortes.

A proposta da história já chama atenção logo no começo: Um walkie-talkie liga passado e presente.\nCasos antigos são reabertos.\nSuspense brilhante.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Signal reúne nomes como Lee Je-hoon, Kim Hye-soo, Cho Jin-woong, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Jang Hyun-sung também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Quando o elenco funciona bem, a história ganha mais força e os personagens acabam ficando bem mais memoráveis.`
},
{
 nome:"Kingdom",
 tags:["intenso","triste"],
 nota:"8.9/10 – MyDramaList",
 elenco:["Ju Ji-hoon", "Bae Doona", "Ryu Seung-ryong", "Kim Sang-ho", "Kim Sung-kyu"],
 descricao:"Uma praga transforma pessoas em monstros.\nUm príncipe luta para salvar o reino.\nAção histórica com terror.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/kngdom.webp",
 trailer:"https://www.youtube.com/embed/4l-yByZpaaM",
 resumo:`Durante a era Joseon,
uma praga transforma pessoas em mortos-vivos.

O príncipe herdeiro tenta descobrir a verdade.
Ele enfrenta traições e conspirações políticas.

Mistura zumbi com drama histórico.
Tem ação intensa e cenas fortes.

É sombrio e muito bem produzido.`
,
 criticaHumana:`Kingdom é o tipo de dorama que costuma agradar quem procura uma história envolvente e personagens fortes.

A proposta da história já chama atenção logo no começo: Uma praga transforma pessoas em monstros.\nUm príncipe luta para salvar o reino.\nAção histórica com terror.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Kingdom reúne nomes como Ju Ji-hoon, Bae Doona, Ryu Seung-ryong, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Kim Sang-ho também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Quando o elenco funciona bem, a história ganha mais força e os personagens acabam ficando bem mais memoráveis.`
},
{
 nome:"Sweet Home",
 tags:["intenso","triste"],
 nota:"8.3/10 – MyDramaList",
 elenco:["Song Kang", "Lee Jin-wook", "Lee Si-young", "Lee Do-hyun", "Go Min-si"],
 descricao:"Humanos se transformam em monstros.\nSobrevivência e drama.\nSombrio e intenso.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/sweet%20home.jpg",
 trailer:"https://www.youtube.com/embed/7rI56NmD33Y",
 resumo:`Pessoas começam a se transformar em monstros.
Um grupo fica preso em um prédio.

Eles precisam sobreviver.
Cada um enfrenta seus próprios medos.

É cheio de ação e tensão.
Mostra o lado sombrio da humanidade.

Intenso e visualmente impactante.`
,
 criticaHumana:`Sweet Home é o tipo de dorama que costuma agradar quem procura uma história envolvente e personagens fortes.

A proposta da história já chama atenção logo no começo: Humanos se transformam em monstros.\nSobrevivência e drama.\nSombrio e intenso.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Sweet Home reúne nomes como Song Kang, Lee Jin-wook, Lee Si-young, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Lee Do-hyun também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Quando o elenco funciona bem, a história ganha mais força e os personagens acabam ficando bem mais memoráveis.`
},
{
 nome:"My Name",
 tags:["intenso","triste"],
 nota:"8.7/10 – MyDramaList",
 elenco:["Han So-hee", "Park Hee-soon", "Ahn Bo-hyun", "Kim Sang-ho", "Lee Hak-joo"],
 descricao:"Uma mulher busca vingança.\nEla entra no mundo do crime.\nAção pesada.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/my%20name.jpg",
 trailer:"https://www.youtube.com/embed/MaJ7ZT70ve4",
 resumo:`Após o assassinato do pai,
uma jovem decide se vingar.

Ela entra para uma organização criminosa.
Depois se infiltra na polícia.

Entre mentiras e violência,
ela perde quem realmente é.

É cheio de ação e drama intenso.`
,
 criticaHumana:`My Name é o tipo de dorama que costuma agradar quem procura uma história envolvente e personagens fortes.

A proposta da história já chama atenção logo no começo: Uma mulher busca vingança.\nEla entra no mundo do crime.\nAção pesada.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de My Name reúne nomes como Han So-hee, Park Hee-soon, Ahn Bo-hyun, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Kim Sang-ho também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Quando o elenco funciona bem, a história ganha mais força e os personagens acabam ficando bem mais memoráveis.`
},
{
 nome:"Strongest Deliveryman",
 tags:["alegre","romance"],
 nota:"8.1/10 – MyDramaList",
 elenco:["Go Kyung-pyo", "Chae Soo-bin", "Kim Seon-ho", "Go Won-hee", "Kim Eui-sung"],
 descricao:"Entregadores sonham com uma vida melhor.\nRomance simples e inspirador.\nMotivador.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/strongest%20deliv.jpg",
 trailer:"https://www.youtube.com/embed/nRUWgGGjYl0",
 resumo:`Jovens entregadores trabalham duro todos os dias.
Eles sonham em abrir o próprio negócio.

Entre dificuldades financeiras,
amizade e romance surgem.

Mostra a luta da juventude comum.
É leve, motivador e sincero.`
,
 criticaHumana:`Strongest Deliveryman é uma boa escolha para quem quer um dorama mais leve, divertido e fácil de maratonar.

A proposta da história já chama atenção logo no começo: Entregadores sonham com uma vida melhor.\nRomance simples e inspirador.\nMotivador.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Strongest Deliveryman reúne nomes como Go Kyung-pyo, Chae Soo-bin, Kim Seon-ho, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Go Won-hee também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Youth of May",
 tags:["romance","triste"],
 nota:"8.8/10 – MyDramaList",
 elenco:["Lee Do-hyun", "Go Min-si", "Lee Sang-yi", "Geum Sae-rok", "Oh Man-seok"],
 descricao:"Amor em meio a um momento histórico.\nEscolhas difíceis e dor.\nMuito emocionante.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/youth%20of%20may.webp",
 trailer:"https://www.youtube.com/embed/FU3i94Jl_z0",
 resumo:`Durante o levante de Gwangju,
um estudante de medicina conhece uma enfermeira.

O amor deles cresce rapidamente.
Mas a realidade política ameaça tudo.

É intenso e doloroso.
Uma história de amor em tempos difíceis.`
,
 criticaHumana:`Youth of May costuma funcionar muito bem para quem gosta de romance com bastante emoção e personagens que deixam marca.

A proposta da história já chama atenção logo no começo: Amor em meio a um momento histórico.\nEscolhas difíceis e dor.\nMuito emocionante.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Youth of May reúne nomes como Lee Do-hyun, Go Min-si, Lee Sang-yi, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Geum Sae-rok também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Rookie Historian Goo Hae Ryung",
 tags:["romance","alegre"],
 nota:"8.3/10 – MyDramaList",
 elenco:["Shin Se-kyung", "Cha Eun-woo", "Park Ki-woong", "Lee Ji-hoon", "Kim Yeo-jin"],
 descricao:"Uma mulher desafia padrões sociais.\nEla se torna historiadora.\nRomance histórico leve.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/rockie%20storian.jfif",
 trailer:"https://www.youtube.com/embed/c7ivQq9HzFg",
 resumo:`Em uma época em que mulheres não tinham voz,
Goo Hae Ryung decide virar historiadora.

Ela enfrenta preconceitos.
Conhece um príncipe secreto.

O romance cresce aos poucos.
Leve, divertido e inspirador.`
,
 criticaHumana:`Rookie Historian Goo Hae Ryung é uma boa escolha para quem quer um dorama mais leve, divertido e fácil de maratonar.

A proposta da história já chama atenção logo no começo: Uma mulher desafia padrões sociais.\nEla se torna historiadora.\nRomance histórico leve.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Rookie Historian Goo Hae Ryung reúne nomes como Shin Se-kyung, Cha Eun-woo, Park Ki-woong, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Lee Ji-hoon também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"The Red Sleeve",
 tags:["romance","triste","intenso"],
 nota:"9.0/10 – MyDramaList",
 elenco:["Lee Jun-ho", "Lee Se-young", "Kang Hoon", "Lee Deok-hwa", "Jang Hye-jin"],
 descricao:"Um rei se apaixona por uma dama da corte.\nAmor impossível.\nHistórico emocionante.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/the%20red%20sleeve.jpeg",
 trailer:"https://www.youtube.com/embed/sBvMK4ZcfEU",
 resumo:`Um príncipe herdeiro se apaixona
por uma dama da corte.

Mas ela deseja liberdade.
O amor deles enfrenta dever e tradição.

História baseada em fatos reais.
Emocionante e marcante.`
,
 criticaHumana:`The Red Sleeve costuma funcionar muito bem para quem gosta de romance com bastante emoção e personagens que deixam marca.

A proposta da história já chama atenção logo no começo: Um rei se apaixona por uma dama da corte.\nAmor impossível.\nHistórico emocionante.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de The Red Sleeve reúne nomes como Lee Jun-ho, Lee Se-young, Kang Hoon, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Lee Deok-hwa também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Alchemy of Souls",
 tags:["fantasia","romance","intenso"],
 nota:"9.0/10 – MyDramaList",
 elenco:["Lee Jae-wook", "Jung So-min", "Go Youn-jung", "Hwang Min-hyun", "Shin Seung-ho"],
 descricao:"Almas trocam de corpos.\nMagia e destino se cruzam.\nFantasia envolvente.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/alchemy%20of%20souls.webp",
 trailer:"https://www.youtube.com/embed/50kLmhGpt1s",
 resumo:`Em um mundo mágico,
almas podem trocar de corpo.

Uma assassina fica presa em outro corpo.
Ela ensina um jovem nobre.

Entre magia e romance,
segredos perigosos surgem.

Fantasia épica e envolvente.`
,
 criticaHumana:`Alchemy of Souls chama atenção pela mistura de fantasia com emoção, o que deixa a história mais marcante.

A proposta da história já chama atenção logo no começo: Almas trocam de corpos.\nMagia e destino se cruzam.\nFantasia envolvente.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Alchemy of Souls reúne nomes como Lee Jae-wook, Jung So-min, Go Youn-jung, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Hwang Min-hyun também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Doctor Stranger",
 tags:["romance","intenso"],
 nota:"8.2/10 – MyDramaList",
 elenco:["Lee Jong-suk", "Jin Se-yeon", "Park Hae-jin", "Kang So-ra", "Choi Jung-woo"],
 descricao:"Um médico com passado doloroso.\nAmor e conspirações.\nDrama médico intenso.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/doctor%20stranger.webp",
 trailer:"https://www.youtube.com/embed/_S2IMBFHS-A",
 resumo:`Criado na Coreia do Norte,
um médico genial foge para o Sul.

Ele tenta reencontrar seu grande amor.
Conspirações médicas surgem.

Drama cheio de tensão e romance.`
,
 criticaHumana:`Doctor Stranger é o tipo de dorama que costuma agradar quem procura uma história envolvente e personagens fortes.

A proposta da história já chama atenção logo no começo: Um médico com passado doloroso.\nAmor e conspirações.\nDrama médico intenso.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Doctor Stranger reúne nomes como Lee Jong-suk, Jin Se-yeon, Park Hae-jin, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Kang So-ra também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"The Good Bad Mother",
 tags:["triste","romance"],
 nota:"8.9/10 – MyDramaList",
 elenco:["Ra Mi-ran", "Lee Do-hyun", "Ahn Eun-jin", "Yoo In-soo", "Jung Woong-in"],
 descricao:"Relação difícil entre mãe e filho.\nSegundas chances.\nEmocionante.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/the%20good%20bad%20mother.jpg",
 trailer:"https://www.youtube.com/embed/gBGW_5oo0Vc",
 resumo:`Uma mãe rígida cria o filho com severidade.
Anos depois, um acidente muda tudo.

Ele perde a memória.
Os dois precisam reconstruir a relação.

Emocionante e cheio de aprendizado.`
,
 criticaHumana:`The Good Bad Mother costuma funcionar muito bem para quem gosta de romance com bastante emoção e personagens que deixam marca.

A proposta da história já chama atenção logo no começo: Relação difícil entre mãe e filho.\nSegundas chances.\nEmocionante.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de The Good Bad Mother reúne nomes como Ra Mi-ran, Lee Do-hyun, Ahn Eun-jin, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Yoo In-soo também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Bad and Crazy",
 tags:["alegre","intenso"],
 nota:"8.4/10 – MyDramaList",
 elenco:["Lee Dong-wook", "Wi Ha-joon", "Han Ji-eun", "Cha Hak-yeon", "Sung Ji-ru"],
 descricao:"Um policial corrupto enfrenta seu outro lado.\nAção e humor.\nDiferente e divertido.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/bad%20and%20crazy.webp",
 trailer:"https://www.youtube.com/embed/kKu6IALrWJE",
 resumo:`Um policial ambicioso começa a ver
um misterioso justiceiro.

Ele descobre que esse homem
é uma parte de sua própria mente.

Ação, humor e muita loucura.
Diferente e viciante.`
,
 criticaHumana:`Bad and Crazy é uma boa escolha para quem quer um dorama mais leve, divertido e fácil de maratonar.

A proposta da história já chama atenção logo no começo: Um policial corrupto enfrenta seu outro lado.\nAção e humor.\nDiferente e divertido.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Bad and Crazy reúne nomes como Lee Dong-wook, Wi Ha-joon, Han Ji-eun, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Cha Hak-yeon também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Quando o elenco funciona bem, a história ganha mais força e os personagens acabam ficando bem mais memoráveis.`
},
{
 nome:"Romance Is a Bonus Book",
 tags:["romance","alegre"],
 nota:"8.6/10 – MyDramaList",
 elenco:["Lee Na-young", "Lee Jong-suk", "Wi Ha-joon", "Jung Yoo-jin", "Kim Tae-woo"],
 descricao:"Uma mulher recomeça a carreira.\nAmor nasce no trabalho.\nDoce e inspirador.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/romance%20is%20a%20bonus%20book.jpg",
 trailer:"https://www.youtube.com/embed/zjeXZEtEI2A",
 resumo:`Uma mulher divorciada tenta voltar ao mercado.
Ela consegue emprego em uma editora.

Lá reencontra um amigo de infância.
O amor nasce devagar.

Doce e confortável de assistir.`
,
 criticaHumana:`Romance Is a Bonus Book é uma boa escolha para quem quer um dorama mais leve, divertido e fácil de maratonar.

A proposta da história já chama atenção logo no começo: Uma mulher recomeça a carreira.\nAmor nasce no trabalho.\nDoce e inspirador.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Romance Is a Bonus Book reúne nomes como Lee Na-young, Lee Jong-suk, Wi Ha-joon, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Jung Yoo-jin também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Navillera",
 tags:["triste","alegre"],
 nota:"8.9/10 – MyDramaList",
 elenco:["Park In-hwan", "Song Kang", "Na Moon-hee", "Hong Seung-hee", "Kim Tae-hoon"],
 descricao:"Um senhor sonha em aprender balé.\nAmizade entre gerações.\nLindo e sensível.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/navillera.webp",
 trailer:"https://www.youtube.com/embed/ZrR8RrltK5Y",
 resumo:`Um senhor de 70 anos decide aprender balé.
Ele encontra um jovem talentoso e perdido.

Os dois criam uma amizade linda.
Mostra sonhos em qualquer idade.

Muito sensível e emocionante.`
,
 criticaHumana:`Navillera é uma boa escolha para quem quer um dorama mais leve, divertido e fácil de maratonar.

A proposta da história já chama atenção logo no começo: Um senhor sonha em aprender balé.\nAmizade entre gerações.\nLindo e sensível.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Navillera reúne nomes como Park In-hwan, Song Kang, Na Moon-hee, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Hong Seung-hee também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Quando o elenco funciona bem, a história ganha mais força e os personagens acabam ficando bem mais memoráveis.`
},
{
 nome:"D.P.",
 tags:["intenso","triste"],
 nota:"8.8/10 – MyDramaList",
 elenco:["Jung Hae-in", "Koo Kyo-hwan", "Kim Sung-kyun", "Son Suk-ku", "Cho Hyun-chul"],
 descricao:"Soldados caçam desertores do exército.\nRealidade dura.\nDrama forte.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/d%20p.jpg",
 trailer:"https://www.youtube.com/embed/ru0PmaWoHxM",
 resumo:`Um soldado entra para a equipe
que captura desertores.

Ele descobre abusos dentro do exército.
Cada caso revela injustiças.

Realista e impactante.`
,
 criticaHumana:`D.P. é o tipo de dorama que costuma agradar quem procura uma história envolvente e personagens fortes.

A proposta da história já chama atenção logo no começo: Soldados caçam desertores do exército.\nRealidade dura.\nDrama forte.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de D.P. reúne nomes como Jung Hae-in, Koo Kyo-hwan, Kim Sung-kyun, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Son Suk-ku também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Quando o elenco funciona bem, a história ganha mais força e os personagens acabam ficando bem mais memoráveis.`
},
{
 nome:"Move to Heaven",
 tags:["triste","alegre"],
 nota:"9.1/10 – MyDramaList",
 elenco:["Tang Joon-sang", "Lee Je-hoon", "Hong Seung-hee", "Ji Jin-hee", "Im Won-hee"],
 descricao:"Jovens lidam com histórias de pessoas falecidas.\nLuto e empatia.\nExtremamente tocante.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/move%20to%20heaven.webp",
 trailer:"https://www.youtube.com/embed/A-er_AJioOA",
 resumo:`Um jovem com Asperger trabalha limpando
pertences de pessoas falecidas.

Com seu tio, ele descobre histórias escondidas.
Cada episódio traz uma lição.

Extremamente sensível e humano.`
,
 criticaHumana:`Move to Heaven é uma boa escolha para quem quer um dorama mais leve, divertido e fácil de maratonar.

A proposta da história já chama atenção logo no começo: Jovens lidam com histórias de pessoas falecidas.\nLuto e empatia.\nExtremamente tocante.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Move to Heaven reúne nomes como Tang Joon-sang, Lee Je-hoon, Hong Seung-hee, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Ji Jin-hee também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Quando o elenco funciona bem, a história ganha mais força e os personagens acabam ficando bem mais memoráveis.`
},
{
 nome:"Little Women",
 tags:["intenso","triste"],
 nota:"8.8/10 – MyDramaList",
 elenco:["Kim Go-eun", "Nam Ji-hyun", "Park Ji-hu", "Wi Ha-joon", "Uhm Ji-won"],
 descricao:"Três irmãs enfrentam uma família poderosa.\nSegredos e corrupção.\nSuspense moderno.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/little%20woman.jpg",
 trailer:"https://www.youtube.com/embed/Y52PRIHr-YI",
 resumo:`Três irmãs pobres se envolvem
em um caso milionário misterioso.

Elas enfrentam uma família poderosa.
Segredos perigosos vêm à tona.

Suspense moderno e elegante.`
,
 criticaHumana:`Little Women é o tipo de dorama que costuma agradar quem procura uma história envolvente e personagens fortes.

A proposta da história já chama atenção logo no começo: Três irmãs enfrentam uma família poderosa.\nSegredos e corrupção.\nSuspense moderno.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Little Women reúne nomes como Kim Go-eun, Nam Ji-hyun, Park Ji-hu, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Wi Ha-joon também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Quando o elenco funciona bem, a história ganha mais força e os personagens acabam ficando bem mais memoráveis.`
},
{
 nome:"Reborn Rich",
 tags:["intenso"],
 nota:"9.0/10 – MyDramaList",
 elenco:["Song Joong-ki", "Lee Sung-min", "Shin Hyun-bin", "Kim Nam-hee", "Park Ji-hyun"],
 descricao:"Um homem renasce em família rica.\nBusca vingança.\nDrama corporativo forte.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/reborn%20ric.jfif",
 trailer:"https://www.youtube.com/embed/6lGImvjI_Sw",
 resumo:`Após ser traído e morto,
um homem renasce como herdeiro da família rica.

Ele planeja vingança.
Manipula negócios e poder.

Drama corporativo cheio de estratégia.`
,
 criticaHumana:`Reborn Rich é o tipo de dorama que costuma agradar quem procura uma história envolvente e personagens fortes.

A proposta da história já chama atenção logo no começo: Um homem renasce em família rica.\nBusca vingança.\nDrama corporativo forte.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Reborn Rich reúne nomes como Song Joong-ki, Lee Sung-min, Shin Hyun-bin, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Kim Nam-hee também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Quando o elenco funciona bem, a história ganha mais força e os personagens acabam ficando bem mais memoráveis.`
},
{
 nome:"Big Mouth",
 tags:["intenso","romance"],
 nota:"8.6/10 – MyDramaList",
 elenco:["Lee Jong-suk", "Im Yoon-ah", "Kim Joo-hun", "Yang Kyung-won", "Kwak Dong-yeon"],
 descricao:"Um advogado vira lenda do crime.\nConspirações perigosas.\nSuspense eletrizante.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/big%20mouth.webp",
 trailer:"https://www.youtube.com/embed/q2bDXqjyxSk",
 resumo:`Um advogado comum é acusado
de ser um grande criminoso.

Ele precisa sobreviver na prisão.
Sua esposa luta para provar sua inocência.

Suspense cheio de tensão.`
,
 criticaHumana:`Big Mouth é o tipo de dorama que costuma agradar quem procura uma história envolvente e personagens fortes.

A proposta da história já chama atenção logo no começo: Um advogado vira lenda do crime.\nConspirações perigosas.\nSuspense eletrizante.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Big Mouth reúne nomes como Lee Jong-suk, Im Yoon-ah, Kim Joo-hun, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Yang Kyung-won também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Queen of Tears",
  nomePT:"Rainha das Lágrimas",
  nomeEN:"Queen of Tears",
 tags:["romance","triste"],
 nota:"9.2/10 – MyDramaList",
 elenco:["Kim Soo-hyun", "Kim Ji-won", "Park Sung-hoon", "Kwak Dong-yeon", "Lee Joo-bin"],
 descricao:"Um casal enfrenta crise profunda.\nAmor, dor e reconciliação.\nMuito emocionante.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/queen%20of%20tears.webp",
 trailer:"https://www.youtube.com/embed/_rNtXO6JuaQ",
 resumo:`Um casal rico enfrenta crise no casamento.
O amor parece ter acabado.

Mas um acontecimento muda tudo.
Eles precisam redescobrir sentimentos.

Romântico e muito emocionante.

No Brasil, esse dorama também é conhecido como "Rainha das Lágrimas".`
,
 criticaHumana:`Rainha das Lágrimas costuma funcionar muito bem para quem gosta de romance com bastante emoção e personagens que deixam marca.

A proposta da história já chama atenção logo no começo: Um casal enfrenta crise profunda.\nAmor, dor e reconciliação.\nMuito emocionante.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Rainha das Lágrimas reúne nomes como Kim Soo-hyun, Kim Ji-won, Park Sung-hoon, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Kwak Dong-yeon também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
  {
    nome:"Sorriso Real (King the Land)",
    nomeEN:"King the Land",
    nomePT:"Sorriso Real",
    tags:["romance","alegre"],
    nota:"8.7/10 – MyDramaList",
 elenco:["Lee Jun-ho", "Im Yoon-ah", "Go Won-hee", "Kim Ga-eun", "Ahn Se-ha"],
    descricao:"Um herdeiro de hotel e uma funcionária cheia de energia vivem um romance fofo no meio de mal-entendidos e contratos.",
    imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/sorriso%20real.webp",
    trailer:"https://www.youtube.com/embed/AGF16szMOmo",
    resumo:`Cheon Sa Rang trabalha em um hotel de luxo e ama o que faz.
Ela conhece Gu Won, um herdeiro frio e sério.
Os dois começam brigando por tudo, mas aos poucos o clima muda.
O dorama é cheio de cenas fofas, humor e muita química.
Perfeito pra quem gosta de romances leves e divertidos.

No Brasil, esse dorama também é conhecido como "Sorriso Real".`
  ,
 criticaHumana:`Sorriso Real é uma boa escolha para quem quer um dorama mais leve, divertido e fácil de maratonar.

A proposta da história já chama atenção logo no começo: Um herdeiro de hotel e uma funcionária cheia de energia vivem um romance fofo no meio de mal-entendidos e contratos.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Sorriso Real reúne nomes como Lee Jun-ho, Im Yoon-ah, Go Won-hee, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Kim Ga-eun também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
    {
    nome:"Vinte e Cinco, Vinte e Um (Twenty-Five Twenty-One)",
    nomeEN:"Twenty-Five Twenty-One",
    nomePT:"Vinte e Cinco, Vinte e Um",
    tags:["romance","triste"],
    nota:"8.9/10 – MyDramaList",
 elenco:["Kim Tae-ri", "Nam Joo-hyuk", "Bona", "Choi Hyun-wook", "Lee Joo-myung"],
    descricao:"Um romance juvenil entre uma esgrimista cheia de sonhos e um jovem que perdeu tudo por causa de uma crise financeira.",
    imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/vinte%20e%20cinto.jpg",
    trailer:"https://www.youtube.com/embed/TuffFHNmGrM",
    resumo:`Na Hee Do é uma esgrimista determinada.
Baek Yi Jin é um jovem que viu a família perder tudo.
Eles se conhecem na juventude e criam uma ligação muito forte.
O dorama mostra amizade, sonhos, amadurecimento e um amor marcante.
É emocionante e mexe muito com a nostalgia.

No Brasil, esse dorama também é conhecido como "Vinte e Cinco, Vinte e Um".`
  ,
 criticaHumana:`Vinte e Cinco, Vinte e Um costuma funcionar muito bem para quem gosta de romance com bastante emoção e personagens que deixam marca.

A proposta da história já chama atenção logo no começo: Um romance juvenil entre uma esgrimista cheia de sonhos e um jovem que perdeu tudo por causa de uma crise financeira.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Vinte e Cinco, Vinte e Um reúne nomes como Kim Tae-ri, Nam Joo-hyuk, Bona, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Choi Hyun-wook também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
  {
    nome:"The Glory",
    tags:["drama","vinganca","intenso"],
    nota:"8.9/10 – MyDramaList",
    elenco:["Song Hye-kyo", "Lee Do-hyun", "Lim Ji-yeon", "Yeom Hye-ran", "Park Sung-hoon"],
    descricao:"Uma mulher marcada por bullying cruel na escola.\nAnos depois, ela volta decidida a se vingar de quem destruiu sua vida.",
    imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/the%20glory.webp",
    trailer:"https://www.youtube.com/embed/UpwtUNagGqI",
    resumo:`Moon Dong Eun sofreu agressões brutais e humilhações constantes na época do colégio.
Ninguém a protegeu, nem a escola, nem os adultos ao redor.

Anos depois, ela volta para a vida dos antigos agressores.
Fria e calculista, ela se aproxima deles assumindo um novo papel e começa a desmontar cada um, pouco a pouco.

O dorama mostra bullying pesado, trauma, culpa e a busca por justiça em um sistema desigual.
É intenso, tenso e cheio de reviravoltas, ótimo para quem gosta de história de vingança bem construída.`
  ,
 criticaHumana:`The Glory é o tipo de dorama que costuma agradar quem procura uma história envolvente e personagens fortes.

A proposta da história já chama atenção logo no começo: Uma mulher marcada por bullying cruel na escola.\nAnos depois, ela volta decidida a se vingar de quem destruiu sua vida.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de The Glory reúne nomes como Song Hye-kyo, Lee Do-hyun, Lim Ji-yeon, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Yeom Hye-ran também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Quando o elenco funciona bem, a história ganha mais força e os personagens acabam ficando bem mais memoráveis.`
},
  {
    nome:"Mr. Queen",
    tags:["comedia","fantasia","historico"],
    nota:"9.0/10 – MyDramaList",
    elenco:["Shin Hye-sun", "Kim Jung-hyun", "Seol In-ah", "Na In-woo", "Bae Jong-ok"],
    descricao:"Um chef moderno acorda preso no corpo de uma rainha da era Joseon.\nConfusão, política e muita comédia em um só dorama.",
    imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/mr%20queen.webp",
    trailer:"https://www.youtube.com/embed/H1vplUsGDoI",
    resumo:`Jang Bong Hwan é um chef de cozinha talentoso nos tempos atuais.
Após um acidente misterioso, sua alma acaba presa no corpo da Rainha Cheorin, na era Joseon.

Preso em um corpo feminino e em um palácio cheio de intrigas, ele precisa se adaptar às regras da corte.
Enquanto tenta sobreviver, acaba se envolvendo em política, conspirações e em um romance inesperado com o rei.

O dorama mistura comédia absurda, romance, drama histórico e momentos emocionantes.
É perfeito para quem gosta de histórias divertidas, mas com bastante conteúdo e personagens marcantes.`
  ,
 criticaHumana:`Mr. Queen é uma boa escolha para quem quer um dorama mais leve, divertido e fácil de maratonar.

A proposta da história já chama atenção logo no começo: Um chef moderno acorda preso no corpo de uma rainha da era Joseon.\nConfusão, política e muita comédia em um só dorama.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Mr. Queen reúne nomes como Shin Hye-sun, Kim Jung-hyun, Seol In-ah, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Na In-woo também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Quando o elenco funciona bem, a história ganha mais força e os personagens acabam ficando bem mais memoráveis.`
},
  {
    nome:"Marry My Husband",
    tags:["vinganca","fantasia","romance"],
    nota:"8.8/10 – MyDramaList",
    elenco:["Park Min-young", "Na In-woo", "Lee Yi-kyung", "Song Ha-yoon", "Lee Gi-kwang"],
    descricao:"Uma mulher traída pelo marido e pela melhor amiga ganha a chance de voltar no tempo.\nDessa vez, ela decide mudar o próprio destino.",
    imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/marry%20my%20husband.webp",
    trailer:"https://www.youtube.com/embed/lfJGSxXf9Xg",
    resumo:`Kang Ji Won descobre que o marido a trai com sua melhor amiga.
Depois de sofrer uma grande injustiça, ela acaba morrendo.

De forma misteriosa, Ji Won volta dez anos no passado, antes de casar.
Com essa segunda chance, ela decide não repetir os mesmos erros e começa a preparar uma vingança planejada.

Enquanto reorganiza sua vida e seu futuro, uma nova possibilidade de amor aparece.
O dorama mistura romance, drama, fantasia e vingança, com uma história viciante e cheia de viradas.`
  ,
 criticaHumana:`Marry My Husband chama atenção pela mistura de fantasia com emoção, o que deixa a história mais marcante.

A proposta da história já chama atenção logo no começo: Uma mulher traída pelo marido e pela melhor amiga ganha a chance de voltar no tempo.\nDessa vez, ela decide mudar o próprio destino.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Marry My Husband reúne nomes como Park Min-young, Na In-woo, Lee Yi-kyung, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Song Ha-yoon também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
  {
    nome:"Beyond Evil",
    tags:["suspense","policial","intenso"],
    nota:"9.0/10 – MyDramaList",
    elenco:["Shin Ha-kyun", "Yeo Jin-goo", "Choi Sung-eun", "Choi Dae-hoon", "Chun Ho-jin"],
    descricao:"Dois detetives investigam assassinatos em série ligados ao passado.\nEles precisam descobrir quem é o verdadeiro monstro.",
    imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/beyond%20evil.jpg",
    trailer:"https://www.youtube.com/embed/zQCOdcBAWtU",
    resumo:`Lee Dong Sik é um ex-detetive brilhante que agora leva uma vida tranquila em uma pequena delegacia.
Quando uma série de assassinatos com o mesmo padrão de um caso antigo volta a acontecer, tudo muda.

Ele passa a trabalhar com o jovem detetive Han Joo Won.
Enquanto os dois tentam descobrir a verdade, segredos do passado vão sendo revelados.

O dorama questiona quem é o verdadeiro monstro: o assassino ou a sociedade que permitiu que tudo acontecesse.
É um thriller denso, cheio de tensão, ótimas atuações e roteiro muito bem amarrado.`
  ,
 criticaHumana:`Beyond Evil prende mais pelo clima de tensão, mistério e curiosidade sobre o que vai acontecer em seguida.

A proposta da história já chama atenção logo no começo: Dois detetives investigam assassinatos em série ligados ao passado.\nEles precisam descobrir quem é o verdadeiro monstro.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Beyond Evil reúne nomes como Shin Ha-kyun, Yeo Jin-goo, Choi Sung-eun, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Choi Dae-hoon também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em histórias mais tensas, um elenco convincente ajuda muito a manter o clima de mistério e a fazer cada conflito parecer mais forte.`
},
  {
    nome:"See You in My 19th Life",
    tags:["romance","fantasia","emocionante"],
    nota:"8.6/10 – MyDramaList",
    elenco:["Shin Hye-sun", "Ahn Bo-hyun", "Ha Yoon-kyung", "Ahn Dong-goo", "Cha Chung-hwa"],
    descricao:"Uma mulher lembra de todas as suas vidas passadas.\nNa 19ª vida, ela decide reencontrar o amor que perdeu na anterior.",
    imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/see%20you%20im%20my.webp",
    trailer:"https://www.youtube.com/embed/zgvMWFEfm_0",
    resumo:`Ban Ji Eum é diferente de qualquer pessoa: ela se lembra de todas as suas vidas passadas.
Na sua 18ª vida, ela conhece um menino especial e cria um laço muito forte com ele.

Depois de um acidente trágico, ela reencarna na 19ª vida.
Determinada, ela vai atrás desse amor do passado, agora já crescido, para tentar se aproximar dele novamente.

O dorama mistura romance, fantasia, humor e emoção.
Fala sobre destino, segundas chances e sobre como as conexões verdadeiras podem atravessar o tempo.`
  ,
 criticaHumana:`See You in My 19th Life chama atenção pela mistura de fantasia com emoção, o que deixa a história mais marcante.

A proposta da história já chama atenção logo no começo: Uma mulher lembra de todas as suas vidas passadas.\nNa 19ª vida, ela decide reencontrar o amor que perdeu na anterior.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de See You in My 19th Life reúne nomes como Shin Hye-sun, Ahn Bo-hyun, Ha Yoon-kyung, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Ahn Dong-goo também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
  {
    nome:"Welcome to Samdal-ri",
    tags:["romance","comedia","aconchegante"],
    nota:"8.7/10 – MyDramaList",
    elenco:["Ji Chang-wook", "Shin Hye-sun", "Kim Mi-kyung", "Seo Hyun-chul", "Shin Dong-mi"],
    descricao:"Uma fotógrafa que caiu em desgraça volta para a cidade natal.\nLá, reencontra um antigo amor e tenta recomeçar a vida.",
    imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/welcome%20to%20san.jpg",
    trailer:"https://www.youtube.com/embed/VKMluVzOWNY",
    resumo:`Cho Sam Dal é uma fotógrafa que fez sucesso na cidade grande, mas vê sua carreira desmoronar após um escândalo.
Sem saída, ela volta para Samdal-ri, sua cidade natal à beira-mar.

Lá, ela reencontra Cho Yong Pil, amigo de infância e antigo amor, que nunca saiu da região.
Enquanto tenta se reerguer, Sam Dal revive memórias, cura feridas antigas e redescobre o que realmente importa.

É um dorama aconchegante, com clima de cidade pequena, personagens carismáticos e uma história de segunda chance na vida e no amor.`
  ,
 criticaHumana:`Welcome to Samdal-ri é uma boa escolha para quem quer um dorama mais leve, divertido e fácil de maratonar.

A proposta da história já chama atenção logo no começo: Uma fotógrafa que caiu em desgraça volta para a cidade natal.\nLá, reencontra um antigo amor e tenta recomeçar a vida.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Welcome to Samdal-ri reúne nomes como Ji Chang-wook, Shin Hye-sun, Kim Mi-kyung, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Seo Hyun-chul também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
  {
    nome:"Gyeongseong Creature",
    tags:["suspense","terror","historico"],
    nota:"8.3/10 – MyDramaList",
    elenco:["Park Seo-joon", "Han So-hee", "Claudia Kim", "Wi Ha-joon", "Kim Hae-sook"],
    descricao:"Em 1945, durante a ocupação japonesa, duas pessoas enfrentam uma criatura criada pela ganância humana.",
    imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/gieogsong%20creature.webp",
    trailer:"https://www.youtube.com/embed/X8Noy8MInY0",
    resumo:`A história se passa em Gyeongseong, em 1945, período em que a Coreia estava sob ocupação japonesa.
Jang Tae Sang é um comerciante influente, conhecido por saber de tudo o que acontece na cidade.

Quando pessoas começam a desaparecer em uma instalação misteriosa, ele acaba se envolvendo com Yoon Chae Ok, uma jovem especialista em encontrar pessoas desaparecidas.
Os dois descobrem experimentos secretos que deram origem a uma criatura monstruosa.

O dorama mistura terror, suspense, ação e drama histórico.
Além da criatura, também mostra a crueldade humana e o peso da guerra na vida das pessoas comuns.`
  ,
 criticaHumana:`Gyeongseong Creature prende mais pelo clima de tensão, mistério e curiosidade sobre o que vai acontecer em seguida.

A proposta da história já chama atenção logo no começo: Em 1945, durante a ocupação japonesa, duas pessoas enfrentam uma criatura criada pela ganância humana.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Gyeongseong Creature reúne nomes como Park Seo-joon, Han So-hee, Claudia Kim, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Wi Ha-joon também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em histórias mais tensas, um elenco convincente ajuda muito a manter o clima de mistério e a fazer cada conflito parecer mais forte.`
},
  {
    nome:"A Time Called You",
    tags:["romance","fantasia","drama"],
    nota:"8.4/10 – MyDramaList",
    elenco:["Ahn Hyo-seop", "Jeon Yeo-been", "Kang Hoon", "Rowoon", "Park Hyuk-kwon"],
    descricao:"Uma mulher em luto pelo namorado recebe um misterioso walkman.\nAo ouvi-lo, ela acaba viajando no tempo para 1998.",
    imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/i%20time%20called%20you.webp",
    trailer:"https://www.youtube.com/embed/c_sKheuGE7o",
    resumo:`Han Jun Hee ainda sofre pela morte do namorado, Koo Yeon Jun.
Um dia, ela recebe uma fita cassete e um antigo walkman.

Ao apertar o play, ela acorda em 1998, no corpo de uma estudante chamada Kwon Min Joo.
Lá, ela conhece Nam Si Heon, um garoto que é idêntico ao seu amor do presente.

O dorama mistura viagem no tempo, mistério, romance e drama.
É cheio de emoção, reviravoltas e reflexões sobre destino, perdas e segundas chances.`
  ,
 criticaHumana:`A Time Called You chama atenção pela mistura de fantasia com emoção, o que deixa a história mais marcante.

A proposta da história já chama atenção logo no começo: Uma mulher em luto pelo namorado recebe um misterioso walkman.\nAo ouvi-lo, ela acaba viajando no tempo para 1998.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de A Time Called You reúne nomes como Ahn Hyo-seop, Jeon Yeo-been, Kang Hoon, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Rowoon também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
  {
    nome:"Tale of the Nine Tailed",
    tags:["fantasia","romance","acao"],
    nota:"8.5/10 – MyDramaList",
    elenco:["Lee Dong-wook", "Jo Bo-ah", "Kim Bum", "Kim Yong-ji", "Hwang Hee"],
    descricao:"Um antigo espírito raposa de nove caudas vive no mundo moderno.\nEle reencontra um amor do passado enquanto caça criaturas perigosas.",
    imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/tale%20of%20the%20nine%20talled.jfif",
    trailer:"https://www.youtube.com/embed/VLzgY1JMRss",
    resumo:`Lee Yeon é um gumiho, uma raposa de nove caudas que antes protegia montanhas.
Agora, ele vive no mundo moderno, trabalhando para eliminar criaturas perigosas que ameaçam os humanos.

Quando conhece Nam Ji Ah, uma produtora de TV determinada, ele começa a acreditar que ela pode ser a reencarnação de seu grande amor do passado.
Ao mesmo tempo, seu meio-irmão Lee Rang causa problemas por ressentimento e dor acumulada.

O dorama mistura fantasia, ação, romance e drama familiar.
Tem clima sombrio em alguns momentos, mas também muitas cenas emocionantes e românticas.`
  ,
 criticaHumana:`Tale of the Nine Tailed chama atenção pela mistura de fantasia com emoção, o que deixa a história mais marcante.

A proposta da história já chama atenção logo no começo: Um antigo espírito raposa de nove caudas vive no mundo moderno.\nEle reencontra um amor do passado enquanto caça criaturas perigosas.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Tale of the Nine Tailed reúne nomes como Lee Dong-wook, Jo Bo-ah, Kim Bum, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Kim Yong-ji também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
  {
    nome:"Strangers from Hell",
    tags:["terror","suspense","psicologico"],
    nota:"8.1/10 – MyDramaList",
    elenco:["Im Si-wan", "Lee Dong-wook", "Lee Jung-eun", "Lee Hyun-wook", "Park Jong-hwan"],
    descricao:"Um jovem se muda para uma pensão barata em Seul.\nLogo, ele percebe que há algo muito errado com seus vizinhos.",
    imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/strangest%20from%20hell.webp",
    trailer:"https://www.youtube.com/embed/bc85-DZwopU",
    resumo:`Yoon Jong Woo se muda do interior para Seul em busca de emprego.
Para economizar, ele aluga um quarto em uma pensão antiga e estranha.

Com o tempo, começa a notar comportamentos bizarros e inquietantes nos outros moradores.
Barulhos à noite, olhares estranhos e uma sensação constante de perigo tomam conta do lugar.

Quanto mais ele tenta fugir daquela atmosfera, mais preso ele parece ficar.
O dorama é pesado, claustrofóbico e cheio de tensão psicológica, ideal para quem gosta de histórias sombrias e perturbadoras.`
  ,
 criticaHumana:`Strangers from Hell prende mais pelo clima de tensão, mistério e curiosidade sobre o que vai acontecer em seguida.

A proposta da história já chama atenção logo no começo: Um jovem se muda para uma pensão barata em Seul.\nLogo, ele percebe que há algo muito errado com seus vizinhos.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Strangers from Hell reúne nomes como Im Si-wan, Lee Dong-wook, Lee Jung-eun, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Lee Hyun-wook também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em histórias mais tensas, um elenco convincente ajuda muito a manter o clima de mistério e a fazer cada conflito parecer mais forte.`
}


,
{
 nome:"Crash Course in Romance",
 tags:["romance","comedia","drama"],
 nota:"8.8/10 – MyDramaList",
 elenco:["Jeon Do-yeon","Jung Kyung-ho","Lee Bong-ryun","Oh Eui-sik"],
 descricao:"Uma mãe dona de restaurante se envolve com um professor famoso.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/crash%20course.webp",
 trailer:"https://www.youtube.com/embed/M0roNIisQ5w",
 resumo:`Nam Haeng Seon é uma ex-atleta que hoje administra um pequeno restaurante.
Sua filha entra no competitivo mundo das aulas particulares.
Ela conhece Choi Chi Yeol, um professor famoso e frio.
Os dois começam com conflitos,
mas aos poucos criam uma ligação sincera.
Mistura romance adulto com crítica ao sistema educacional.
Leve, emocionante e muito popular.`
,
 criticaHumana:`Crash Course in Romance é uma boa escolha para quem quer um dorama mais leve, divertido e fácil de maratonar.

A proposta da história já chama atenção logo no começo: Uma mãe dona de restaurante se envolve com um professor famoso.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Crash Course in Romance reúne nomes como Jeon Do-yeon, Jung Kyung-ho, Lee Bong-ryun, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Oh Eui-sik também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"My Dearest",
 tags:["historico","romance","intenso"],
 nota:"8.9/10 – MyDramaList",
 elenco:["Namkoong Min","Ahn Eun-jin","Lee Hak-joo","Lee Da-in"],
 descricao:"Amor em tempos de guerra na era Joseon.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/my%20dearest.jpeg",
 trailer:"https://www.youtube.com/embed/CZlxbotr8Ms",
 resumo:`Durante a invasão Qing,
um homem misterioso e uma jovem nobre se aproximam.
O amor deles nasce em meio à guerra e sofrimento.
A história mostra lealdade, dor e sobrevivência.
Visual cinematográfico e atuação intensa.
Um dos maiores sucessos históricos recentes.`
,
 criticaHumana:`My Dearest é o tipo de dorama que costuma agradar quem procura uma história envolvente e personagens fortes.

A proposta da história já chama atenção logo no começo: Amor em tempos de guerra na era Joseon.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de My Dearest reúne nomes como Namkoong Min, Ahn Eun-jin, Lee Hak-joo, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Lee Da-in também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"The Worst of Evil",
 tags:["acao","crime","intenso"],
 nota:"8.7/10 – MyDramaList",
 elenco:["Ji Chang-wook","Wi Ha-joon","Im Se-mi"],
 descricao:"Um policial se infiltra em cartel de drogas internacional.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/the%20worst%20of%20evil.webp",
 trailer:"https://www.youtube.com/embed/L5qtpG1kqfI",
 resumo:`Um policial aceita missão secreta perigosa.
Ele se infiltra em organização criminosa poderosa.
A tensão cresce conforme sua identidade quase é descoberta.
Ação pesada e atmosfera sombria.
Muito elogiado por realismo e atuações.`
,
 criticaHumana:`The Worst of Evil é o tipo de dorama que costuma agradar quem procura uma história envolvente e personagens fortes.

A proposta da história já chama atenção logo no começo: Um policial se infiltra em cartel de drogas internacional.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de The Worst of Evil reúne nomes como Ji Chang-wook, Wi Ha-joon, Im Se-mi, que ajudam a dar mais peso e carisma para a história.

Quando o elenco funciona bem, a história ganha mais força e os personagens acabam ficando bem mais memoráveis.`
},
{
 nome:"Bloodhounds",
 tags:["acao","crime","intenso"],
 nota:"8.6/10 – MyDramaList",
 elenco:["Woo Do-hwan","Lee Sang-yi","Kim Sae-ron"],
 descricao:"Dois boxeadores enfrentam agiotas violentos.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/bloodhounds.webp",
 trailer:"https://www.youtube.com/embed/eyiR8pO50JE",
 resumo:`Dois jovens entram no mundo perigoso dos empréstimos ilegais.
Eles enfrentam um vilão cruel e poderoso.
Cheio de cenas de luta intensas.
Mostra amizade e lealdade.
Rápido, violento e muito viciante.`
,
 criticaHumana:`Bloodhounds é o tipo de dorama que costuma agradar quem procura uma história envolvente e personagens fortes.

A proposta da história já chama atenção logo no começo: Dois boxeadores enfrentam agiotas violentos.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Bloodhounds reúne nomes como Woo Do-hwan, Lee Sang-yi, Kim Sae-ron, que ajudam a dar mais peso e carisma para a história.

Quando o elenco funciona bem, a história ganha mais força e os personagens acabam ficando bem mais memoráveis.`
},
{
 nome:"Agency",
 tags:["drama","corporativo","intenso"],
 nota:"8.4/10 – MyDramaList",
 elenco:["Lee Bo-young","Son Na-eun","Jo Sung-ha"],
 descricao:"Uma executiva luta para sobreviver em empresa dominada por homens.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/agency.webp",
 trailer:"https://www.youtube.com/embed/UZgXPPo2YmU",
 resumo:`Go Ah In é a primeira mulher a chegar ao topo de uma agência de publicidade.
Ela enfrenta preconceito e competição cruel.
Mostra ambição e política corporativa.
Drama adulto e estratégico.
Muito comentado por sua crítica social.`
,
 criticaHumana:`Agency é o tipo de dorama que costuma agradar quem procura uma história envolvente e personagens fortes.

A proposta da história já chama atenção logo no começo: Uma executiva luta para sobreviver em empresa dominada por homens.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Agency reúne nomes como Lee Bo-young, Son Na-eun, Jo Sung-ha, que ajudam a dar mais peso e carisma para a história.

Quando o elenco funciona bem, a história ganha mais força e os personagens acabam ficando bem mais memoráveis.`
},
{
 nome:"Doctor Cha",
 tags:["romance","drama","superacao"],
 nota:"8.5/10 – MyDramaList",
 elenco:["Uhm Jung-hwa","Kim Byung-chul","Myung Se-bin"],
 descricao:"Uma dona de casa decide retomar carreira médica.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/doctor%20cha.jpg",
 trailer:"https://www.youtube.com/embed/qDjF9Bkib28",
 resumo:`Após anos dedicada à família,
Cha Jung Sook decide voltar à medicina.
Ela enfrenta desafios e redescobre sua identidade.
O dorama fala sobre segunda chance.
Emocionante e inspirador.`
,
 criticaHumana:`Doctor Cha é o tipo de dorama que costuma agradar quem procura uma história envolvente e personagens fortes.

A proposta da história já chama atenção logo no começo: Uma dona de casa decide retomar carreira médica.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Doctor Cha reúne nomes como Uhm Jung-hwa, Kim Byung-chul, Myung Se-bin, que ajudam a dar mais peso e carisma para a história.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Island",
 tags:["fantasia","acao","terror"],
 nota:"8.3/10 – MyDramaList",
 elenco:["Kim Nam-gil","Lee Da-hee","Cha Eun-woo"],
 descricao:"Demônios atacam ilha misteriosa.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/island.webp",
 trailer:"https://www.youtube.com/embed/RJn21G18Ocs",
 resumo:`Uma mulher descobre estar ligada a forças sobrenaturais.
Um caçador de demônios a protege.
Mistura ação e fantasia sombria.
Visual marcante e atmosfera intensa.`
,
 criticaHumana:`Island prende mais pelo clima de tensão, mistério e curiosidade sobre o que vai acontecer em seguida.

A proposta da história já chama atenção logo no começo: Demônios atacam ilha misteriosa.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Island reúne nomes como Kim Nam-gil, Lee Da-hee, Cha Eun-woo, que ajudam a dar mais peso e carisma para a história.

Em histórias mais tensas, um elenco convincente ajuda muito a manter o clima de mistério e a fazer cada conflito parecer mais forte.`
},
{
 nome:"The Devil Judge",
 tags:["suspense","politico","intenso"],
 nota:"8.8/10 – MyDramaList",
 elenco:["Ji Sung","Kim Min-jung","Jinyoung"],
 descricao:"Um juiz transforma julgamentos em reality show nacional.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/the%20devil%20judge.jpg",
 trailer:"https://www.youtube.com/embed/EnJE8_oomP4",
 resumo:`Em um futuro distópico,
julgamentos são transmitidos ao vivo.
Um juiz carismático manipula o sistema.
Questiona justiça e corrupção.
Tenso e provocador.`
,
 criticaHumana:`The Devil Judge prende mais pelo clima de tensão, mistério e curiosidade sobre o que vai acontecer em seguida.

A proposta da história já chama atenção logo no começo: Um juiz transforma julgamentos em reality show nacional.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de The Devil Judge reúne nomes como Ji Sung, Kim Min-jung, Jinyoung, que ajudam a dar mais peso e carisma para a história.

Em histórias mais tensas, um elenco convincente ajuda muito a manter o clima de mistério e a fazer cada conflito parecer mais forte.`
},
{
 nome:"Under the Queen's Umbrella",
 tags:["historico","drama","familia"],
 nota:"8.8/10 – MyDramaList",
 elenco:["Kim Hye-soo","Kim Hae-sook","Choi Won-young"],
 descricao:"Uma rainha tenta proteger seus filhos problemáticos.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/under%20the%20queen%20umbrella.webp",
 trailer:"https://www.youtube.com/embed/PW6oCJXONYk",
 resumo:`A rainha enfrenta intrigas no palácio.
Ela luta para garantir o futuro dos filhos.
Drama histórico focado em maternidade.
Elegante e muito elogiado.`
,
 criticaHumana:`Under the Queen's Umbrella é o tipo de dorama que costuma agradar quem procura uma história envolvente e personagens fortes.

A proposta da história já chama atenção logo no começo: Uma rainha tenta proteger seus filhos problemáticos.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Under the Queen's Umbrella reúne nomes como Kim Hye-soo, Kim Hae-sook, Choi Won-young, que ajudam a dar mais peso e carisma para a história.

Quando o elenco funciona bem, a história ganha mais força e os personagens acabam ficando bem mais memoráveis.`
},
{
 nome:"My Liberation Notes",
 tags:["drama","romance","realista"],
 nota:"8.9/10 – MyDramaList",
 elenco:["Lee Min-ki","Kim Ji-won","Son Seok-koo"],
 descricao:"Três irmãos buscam sentido na vida adulta.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/my%20liberation%20notes.webp",
 trailer:"https://www.youtube.com/embed/FQEm4dPZsNo",
 resumo:`Três irmãos vivem rotina cansativa no interior.
Eles sonham com liberdade emocional.
Um estranho misterioso muda suas vidas.
Profundo, reflexivo e muito humano.`
,
 criticaHumana:`My Liberation Notes é o tipo de dorama que costuma agradar quem procura uma história envolvente e personagens fortes.

A proposta da história já chama atenção logo no começo: Três irmãos buscam sentido na vida adulta.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de My Liberation Notes reúne nomes como Lee Min-ki, Kim Ji-won, Son Seok-koo, que ajudam a dar mais peso e carisma para a história.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
}
,
{
 nome:"Boys Over Flowers",
 nomePT:"Boys Over Flowers",
 nomeEN:"Boys Over Flowers",
 tags:["romance","escolar","triste"],
 nota:"8.1/10 – MyDramaList",
 elenco:["Lee Min-ho","Ku Hye-sun","Kim Hyun-joong","Kim Bum","Kim Joon"],
 descricao:"Uma jovem simples entra em uma escola de elite e se envolve com os garotos mais poderosos do colégio.\nClássico absoluto do romance escolar coreano.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/boys%20over%20flowers.webp",
 trailer:"https://www.youtube.com/embed/RNf7htKrZuc",
 resumo:`Geum Jan Di é uma garota humilde que acaba entrando em uma escola frequentada pelos filhos das famílias mais ricas da Coreia.
Lá ela cruza o caminho do F4, grupo formado pelos quatro garotos mais populares e temidos do colégio.

O líder do grupo é Goo Jun Pyo, arrogante e impulsivo, mas que aos poucos começa a mudar por causa dela.
Ao mesmo tempo, Jan Di se aproxima de Yoon Ji Hoo, o membro mais calmo e sensível do grupo.

O dorama mistura romance, rivalidade, diferenças sociais e muitos conflitos escolares.
Mesmo sendo um clássico mais antigo, continua muito procurado por fãs de doramas escolares e romances coreanos famosos.`
,
 criticaHumana:`Boys Over Flowers costuma funcionar muito bem para quem gosta de romance com bastante emoção e personagens que deixam marca.

A proposta da história já chama atenção logo no começo: Uma jovem simples entra em uma escola de elite e se envolve com os garotos mais poderosos do colégio.\nClássico absoluto do romance escolar coreano.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Boys Over Flowers reúne nomes como Lee Min-ho, Ku Hye-sun, Kim Hyun-joong, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Kim Bum também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"School 2015: Who Are You",
 nomePT:"School 2015: Quem É Você?",
 nomeEN:"School 2015: Who Are You",
 tags:["escolar","romance","suspense"],
 nota:"8.2/10 – MyDramaList",
 elenco:["Kim So-hyun","Nam Joo-hyuk","Yook Sung-jae","Lee Pil-mo","Cho Soo-hyang"],
 descricao:"Duas garotas idênticas têm destinos cruzados em uma escola marcada por bullying e segredos.\nDrama escolar com romance e mistério.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/school%202015%20who%20are%20you.jpg",
 trailer:"https://www.youtube.com/embed/QGzhWpD2gZw",
 resumo:`Go Eun Byul e Lee Eun Bi são gêmeas separadas que levam vidas completamente diferentes.
Quando um acidente misterioso acontece, uma delas assume a vida da outra e passa a estudar em uma escola cheia de conflitos.

O dorama fala sobre bullying, identidade, trauma e amizade, enquanto também constrói um triângulo amoroso marcante.
É um dos títulos escolares mais conhecidos da franquia School e costuma aparecer entre os doramas de escola mais lembrados pelos fãs.`
,
 criticaHumana:`School 2015: Quem É Você? prende mais pelo clima de tensão, mistério e curiosidade sobre o que vai acontecer em seguida.

A proposta da história já chama atenção logo no começo: Duas garotas idênticas têm destinos cruzados em uma escola marcada por bullying e segredos.\nDrama escolar com romance e mistério.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de School 2015: Quem É Você? reúne nomes como Kim So-hyun, Nam Joo-hyuk, Yook Sung-jae, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Lee Pil-mo também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"School 2017",
 nomePT:"School 2017",
 nomeEN:"School 2017",
 tags:["escolar","romance","alegre"],
 nota:"7.8/10 – MyDramaList",
 elenco:["Kim Se-jeong","Kim Jung-hyun","Jang Dong-yoon","Han Joo-wan","Han Sun-hwa"],
 descricao:"Uma estudante tenta sobreviver à pressão escolar enquanto um misterioso aluno causa confusão na escola.\nLeve, jovem e divertido.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/school%202017.jpg",
 trailer:"https://www.youtube.com/embed/WIHqGiztrmc",
 resumo:`Ra Eun Ho é uma estudante alegre que sonha em entrar na faculdade de artes, mas vive lutando contra as cobranças da escola.
Tudo muda quando um misterioso aluno conhecido como X começa a provocar confusões dentro do colégio.

Entre punições, amizades, pressão acadêmica e romance, a história mostra o cotidiano de adolescentes tentando encontrar seu lugar.
É um dorama escolar mais leve e fácil de assistir, bom para quem gosta de juventude, casal fofo e clima colegial.`
,
 criticaHumana:`School 2017 é uma boa escolha para quem quer um dorama mais leve, divertido e fácil de maratonar.

A proposta da história já chama atenção logo no começo: Uma estudante tenta sobreviver à pressão escolar enquanto um misterioso aluno causa confusão na escola.\nLeve, jovem e divertido.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de School 2017 reúne nomes como Kim Se-jeong, Kim Jung-hyun, Jang Dong-yoon, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Han Joo-wan também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Dream High",
 nomePT:"Dream High",
 nomeEN:"Dream High",
 tags:["escolar","romance","alegre"],
 nota:"8.0/10 – MyDramaList",
 elenco:["Suzy","Kim Soo-hyun","Taecyeon","IU","Wooyoung"],
 descricao:"Jovens talentosos entram em uma escola de artes para realizar o sonho de se tornarem estrelas.\nMúsica, romance e juventude.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/dream%20hight.jpg",
 trailer:"https://www.youtube.com/embed/ppMlRQsrvlw",
 resumo:`Dream High acompanha um grupo de jovens que entra em uma escola de artes com o sonho de se tornar cantor, dançarino ou astro da música.
Cada personagem chega com inseguranças, talentos e dificuldades próprias.

Ao longo da história, eles enfrentam rivalidade, amadurecimento, apresentações e romances que nascem dentro da escola.
É um dos doramas escolares musicais mais famosos da Coreia e continua sendo muito lembrado por fãs antigos e por quem gosta de séries juvenis coreanas.`
,
 criticaHumana:`Dream High é uma boa escolha para quem quer um dorama mais leve, divertido e fácil de maratonar.

A proposta da história já chama atenção logo no começo: Jovens talentosos entram em uma escola de artes para realizar o sonho de se tornarem estrelas.\nMúsica, romance e juventude.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Dream High reúne nomes como Suzy, Kim Soo-hyun, Taecyeon, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, IU também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
},
{
 nome:"Moment at Eighteen",
 nomePT:"Moment at Eighteen",
 nomeEN:"Moment at Eighteen",
 tags:["escolar","romance","triste"],
 nota:"8.4/10 – MyDramaList",
 elenco:["Ong Seong-wu","Kim Hyang-gi","Shin Seung-ho","Kang Ki-young","Moonbin"],
 descricao:"Um estudante solitário tenta recomeçar a vida em uma nova escola.\nDrama escolar sensível, romântico e realista.",
 imagem:"https://raw.githubusercontent.com/boymav/clube-do-dorama/refs/heads/main/imagens/doramas/moment%20of%20eighteen.jfif",
 trailer:"https://www.youtube.com/embed/EdCpXOhqw-w",
 resumo:`Choi Joon Woo é um garoto reservado que precisa se adaptar a uma nova escola depois de vários problemas no passado.
Lá ele conhece Yoo Soo Bin, uma aluna brilhante que vive sob forte pressão da mãe, e Ma Hwi Young, o estudante popular da turma.

O dorama mostra amizade, insegurança, amadurecimento e a dor típica dessa fase da vida.
É uma história escolar mais sensível e realista, muito indicada para quem gosta de juventude, romance e desenvolvimento emocional dos personagens.`
,
 criticaHumana:`Moment at Eighteen costuma funcionar muito bem para quem gosta de romance com bastante emoção e personagens que deixam marca.

A proposta da história já chama atenção logo no começo: Um estudante solitário tenta recomeçar a vida em uma nova escola.\nDrama escolar sensível, romântico e realista.

O que faz a série funcionar melhor é como ela desenvolve essa base com mais emoção ao longo dos episódios.

Entre os fãs, é um título que costuma ser lembrado como uma opção forte dentro do seu estilo.`,
 elencoInfo:`O elenco de Moment at Eighteen reúne nomes como Ong Seong-wu, Kim Hyang-gi, Shin Seung-ho, que ajudam a dar mais peso e carisma para a história.

Além dos protagonistas, Kang Ki-young também aparece entre os nomes importantes do dorama, reforçando bem o núcleo principal.

Em um dorama com foco romântico, a química entre os atores faz bastante diferença, e isso costuma influenciar muito na forma como o público se apega ao casal.`
}

];function criarSlug(nome, nomeEN) {
  const base = nomeEN || nome;
  return String(base || "")
    .normalize("NFD").replace(/[\u0300-\u036f]/g, "")
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/(^-|-$)/g, "");
}

doramas.forEach(d => {
  d.slug = criarSlug(d.nome, d.nomeEN);

});
